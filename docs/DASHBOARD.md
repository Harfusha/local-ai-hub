...
Overview shows agent HTTP latency separately from local-inference latency, policy rejections separately from operational failures, and v2.4 token-efficiency accounting. The headline is a signed net cloud-token delta: the strongest measured counterfactual source/context baseline (or response-compaction baseline when no source baseline exists) minus the estimated tokens the cloud agent spends emitting the MCP tool call and reading the projected tool response. Calls that cost more context than they save therefore appear as negative overhead instead of being hidden. Overlapping raw → packed → projected reductions are not added together; the visible tool response is charged once. The enabled-tool schema catalog is shown separately as a conservative schema-adjusted upper-bound because hosts differ in how often schema text is injected/reused. Local cache/single-flight compute avoidance is a separate metric and is never added to cloud-context savings. USD savings use the configurable blended rate `token_saving.cloud_token_cost_usd_per_million` (default: `$3.00` per million net tokens) and are explicitly estimates.
