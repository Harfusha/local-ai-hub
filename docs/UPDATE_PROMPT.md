...
- Keep routing boundaries explicit: repository navigation/symbols/impact=`local_ai_repo`; exact source/log slices=`local_ai_artifact`; test/lint/typecheck/build=`local_ai_command`; ownership/checkpoints/receipts=`local_ai_coord`; local diagnosis/boilerplate/second opinion=`local_ai_task` only after low-confidence deterministic command parsing with artifact reference plus narrow preview, never raw logs; closed, low-risk work=`local_ai_work`.
