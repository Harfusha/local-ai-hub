...
class ReviewDiffChunkingTests(unittest.TestCase):
    def _services(self, diff_text, estimated_tokens, changed_files=None):
        ...
        config = {
            "models": {
                ...
                "general": "qwen2.5-coder:1.5b-instruct-q5_K_M",
            },
    ...
    def test_malformed_model_text_is_not_reported_as_success(self):
        diff_text = "diff --git a/a.py b/a.py\n--- a/a.py\n+++ b/a.py\n@@ -1 +1 @@\n-old\n+new\n"
        services = self._services(diff_text, estimate_tokens(diff_text))
        services.delegate = MagicMock(side_effect=[
            {"success": True, "text": "1**", "model": "7b"},
            {"success": True, "text": "1**", "model": "qwen2.5-coder:1.5b-instruct-q5_K_M"},
        ])
        ...
        self.assertEqual(services.delegate.call_count, 2)
        retry_payload = services.delegate.call_args_list[1].args[0]
        self.assertEqual(retry_payload["model"], "qwen2.5-coder:1.5b-instruct-q5_K_M")
        self.assertIn("FORMAT RECOVERY", retry_payload["task"])

    def test_malformed_primary_output_recovers_with_fast_model(self):
        diff_text = "diff --git a/a.py b/a.py\n--- a/a.py\n+++ b/a.py\n@@ -1 +1 @@\n-old\n+new\n"
        valid_text = "SUMMARY: Review completed; no actionable findings found."
        services = self._services(diff_text, estimate_tokens(diff_text))
        services.delegate = MagicMock(side_effect=[
            {"success": True, "text": "1**", "model": "qwen2.5-coder:3b-instruct-q5_K_M"},
            {"success": True, "text": valid_text, "model": "qwen2.5-coder:1.5b-instruct-q5_K_M"},
        ])

        result = LocalAIServices.review_diff(services, {"root": "."}, "test")

        self.assertTrue(result["success"])
        self.assertEqual(result["text"], valid_text)
        self.assertTrue(result["degraded"])
        self.assertEqual(result["format_recoveries"], [{
            "segment": 1,
            "from_model": "qwen2.5-coder:3b-instruct-q5_K_M",
            "to_model": "qwen2.5-coder:1.5b-instruct-q5_K_M",
        }])
        self.assertEqual(services.delegate.call_count, 2)

    def test_malformed_fast_model_escalates_to_heavy_model(self):
        diff_text = "diff --git a/a.py b/a.py\n--- a/a.py\n+++ b/a.py\n@@ -1 +1 @@\n-old\n+new\n"
        services = self._services(diff_text, estimate_tokens(diff_text))
        services.delegate = MagicMock(side_effect=[
            {"success": True, "text": "1**", "model": "qwen2.5-coder:1.5b"},
            {"success": True, "text": "SUMMARY: Review completed; no actionable findings found.", "model": "qwen2.5-coder:3b"},
        ])

        result = LocalAIServices.review_diff(services, {"root": ".", "base": "HEAD"}, "default")

        self.assertTrue(result["success"])
        self.assertEqual(services.delegate.call_count, 2, repr(result))
        retry_payload = services.delegate.call_args_list[1].args[0]
        self.assertEqual(retry_payload["model"], "qwen2.5-coder:3b-instruct-q5_K_M")

    def test_delegate_exception_returns_structured_failure(self):
        diff_text = "diff --git a/a.py b/a.py\n--- a/a.py\n+++ b/a.py\n@@ -1 +1 @@\n-old\n+new\n"
        services = self._services(diff_text, estimate_tokens(diff_text))
        services.delegate = MagicMock(side_effect=RuntimeError("sensitive internal detail"))

        result = LocalAIServices.review_diff(services, {"root": "."}, "test")

        self.assertFalse(result["success"])
        self.assertEqual(result["error_type"], "RuntimeError")
        self.assertNotIn("sensitive internal detail", result["error"])
        self.assertEqual(services.delegate.call_count, 1)
    ...
    def test_many_small_files_are_packed_by_tokens_not_file_count(self):
        ...
        self.assertEqual(result["diff"]["review_chunks"], 1)
        self.assertEqual(result["diff"]["review_model"], "qwen2.5-coder:3b-instruct-q5_K_M")
