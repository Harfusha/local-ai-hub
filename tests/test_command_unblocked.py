    ...
    return CommandBroker(
        {
            ...
            "features": {"diagnostic_artifacts": True, "local_diagnostic_dispatch": True},
        },
        _Artifacts(),
        _RepoState(),
    )
...
def test_model_error_distiller_only_runs_when_parser_has_no_diagnostic(tmp_path):
    ...
    res = broker.run(f'{sys.executable} -c "print(\'opaque signal\'); raise SystemExit(7)"', str(tmp_path), "tenant")
