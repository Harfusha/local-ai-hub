...
- Model routing is profile-aware: integrated uses qwen2.5-coder:0.5b preprocessing, qwen2.5-coder:1.5b quick, qwen2.5-coder:3b involved, qwen2.5-coder:7b hard reasoning and qwen3-vl:4b vision; balanced uses qwen2.5-coder:1.5b preprocessing, qwen2.5-coder:3b fast, qwen2.5-coder:7b ordinary/hard work and qwen3.5:9b only for extreme reasoning/vision; high uses 1.5b/3b/7b/9b; max uses 1.5b/3b/9b throughout its heavy routes. Keep deterministic simple tasks enabled for exact facts, symbols, diff and tests; use `local_ai_task` for semantic generation, reasoning, review, independent second opinions and compression.
...
Default guarded context flow: Before planning, editing, reviewing, testing, or handing off non-trivial repository work, call `local_ai_repo(action="context")` with the stable absolute root and the current phase (`plan`, `edit`, `review`, `test`, or `handoff`). Include `task_id`, focus, and `preload_profile` when known; cite returned `evidence_ids`. Reuse candidates before introducing new abstractions. Treat deterministic/indexed evidence as authoritative; local-model output may rank or compose it but cannot override it. If a preload, code-intelligence backend, local model, or Agent OS layer is unavailable, continue with the useful deterministic/indexed or stateless pack and its concise machine-readable warning. Preserve legacy fast/full behavior only for callers that omit guarded fields.
