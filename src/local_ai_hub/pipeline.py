...
class LocalAgentPipeline:
    ...
    def solve_repo(self, args: dict[str, Any], tenant: str) -> dict[str, Any]:
        ...
        if (
            bool(plan.get("explorer"))
            and not explorer.get("success")
            and self.tool_agent is not None
            and not explorer.get("repetition_loop_detected")
        ):
        ...
        if not explorer.get("success"):
            if explorer.get("repetition_loop_detected"):
                return {
                    "success": False,
                    "error": "local reasoning stopped after repetitive model output",
                    "pipeline": {"mode": mode, "degraded": True, "failed_stage": "explorer"},
                }
        ...
        if need_worker:
            ...
            def compute_worker() -> dict[str, Any]:
                if self.tool_agent is not None:
                    ...
                    if tool_result.get("success"):
                        return tool_result
                    if tool_result.get("repetition_loop_detected"):
                        return tool_result
            ...
            if not worker.get("success"):
                if worker.get("repetition_loop_detected"):
                    return {
                        "success": False,
                        "error": "local reasoning stopped after repetitive model output",
                        "pipeline": {"mode": mode, "degraded": True, "failed_stage": "worker"},
                    }
                fallback = self.services.delegate_repo(args, tenant)
                ...
                def compute_refine() -> dict[str, Any]:
                    if self.tool_agent is not None:
                        ...
                        if refined.get("success"):
                            return refined
                        if refined.get("repetition_loop_detected"):
                            return refined
                    return self.services._generate(
                        worker_model,
                        f"TASK:\n{task}\n\nFIRST PASS:\n{json.dumps(first_state, ensure_ascii=False, separators=(',',':'))}\n\nEVIDENCE:\n{packed.get('context','')}",
