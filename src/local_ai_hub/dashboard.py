...
function traceCodexTimeline(model,title='Model chat',renderBudget=null,full=false){
  ...
  const cards=events.map((event,index)=>full?traceCodexEventMarkup(event,index,null):traceBudgetMarkup(traceCodexEventMarkup(event,index,budget),budget)).join(''),hasOutput=events.some(event=>['output_delta','output_stream','assistant_output','model_output','output'].includes(String(event?.event_type||'').toLowerCase())),outputState=traceCaptureState(presentation?.modelOutput,presentation?.finalResponseStatus??model.finalResponseStatus??model.session?.final_response_status??'');
