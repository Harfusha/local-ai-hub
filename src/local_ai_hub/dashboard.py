...
function tracePresentationColumns(title,left,right,budget){const leftMarkup=left.join(''),rightMarkup=right.join('');return traceFinalizeMarkup(`<section class="trace-primary trace-request-presentation"><div class="trace-primary-head"><h2>${esc(title)}</h2></div><div class="trace-chat-columns trace-primary-grid"><article class="trace-primary-card input"><h3>Request details</h3><div class="trace-primary-value">${leftMarkup||'<div class="trace-primary-empty">No request details captured</div>'}</div></article><article class="trace-primary-card output"><h3>Result details</h3><div class="trace-primary-value">${rightMarkup||'<div class="trace-primary-empty">No result details captured</div>'}</div></article></div></section>`,budget);}
function renderCommandPresentation(model){
  const budget=traceRenderBudget(),command=tracePresentationPayload(model,'command'),safe=traceSanitizeValue(command),left=[tracePresentationField('command',tracePresentationPick(safe,['command','cmd'],safe),'No command captured',budget),tracePresentationField('args',tracePresentationPick(safe,['args','arguments'],'',),'No args captured',budget),tracePresentationField('input',tracePresentationPick(safe,['input','stdin'],'',),'No input captured',budget)],right=[tracePresentationField('stdout',safe.stdout,'No stdout captured',budget),tracePresentationField('stderr',safe.stderr,'No stderr captured',budget),tracePresentationField('exit code',tracePresentationPick(safe,['exit_code','exitCode','code'],''),'No exit code captured',budget),tracePresentationField('retries',tracePresentationPick(safe,['retries','retry_count','attempts'],''),'No retries captured',budget),tracePresentationField('duration',tracePresentationPick(safe,['duration','duration_ms','elapsed_ms'],''),'No duration captured',budget)];
  return tracePresentationColumns('Command',left,right,budget);
}
function renderReviewPresentation(model){
  ...
  return tracePresentationColumns('Review',left,right,budget);
}
...
function renderRagSearchPresentation(model){
  const budget=traceRenderBudget(),retrieval=traceSanitizeValue(tracePresentationPayload(model,'retrieval')),left=[tracePresentationField('query',retrieval.query,'No query captured',budget),tracePresentationField('sources',retrieval.sources,'No sources captured',budget),tracePresentationField('results',retrieval.results??retrieval.hits,'No results captured',budget),tracePresentationField('score',retrieval.score??retrieval.scores,'No score captured',budget),tracePresentationField('provider',retrieval.provider,'No provider captured',budget)],right=[tracePresentationField('answer',retrieval.answer,'No answer captured',budget),tracePresentationField('truncation',retrieval.truncated??retrieval.truncation,'No truncation recorded',budget)];
