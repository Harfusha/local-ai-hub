...
function traceNormalizeEvent(event){
  if(event&&typeof event==='object'&&!Array.isArray(event)){const payload=event.payload&&typeof event.payload==='object'&&!Array.isArray(event.payload)?event.payload:{};return {...event,payload};}
  return {event_type:'malformed',payload:{value:event===undefined?null:event}};
}
...
function traceSummary(model,unavailableCopy){
 ...
 function renderTraceDetail(d){
  ...
  const panels=model.panels.filter(panel=>panel.available),universalSummary=traceSummary(model,unavailableCopy),panelMarkup=panels.map(panel=>{const selected=panel.id===traceView,content=selected?(panel.id==='summary'?'<div class="empty-human">Summary shown above.</div>':panel.render()):'';return `<div id="trace-panel-${esc(panel.id)}" class="trace-view" role="tabpanel" aria-labelledby="trace-tab-${esc(panel.id)}"${selected?'':' hidden'}>${content}</div>`}).join('');
   const primaryInput=model.availability.input?renderAny(model.input):`<span class="trace-primary-empty">${esc(unavailableCopy.input)}</span>`;
   const primaryOutput=model.availability.output?renderAny(model.output):`<span class="trace-primary-empty">${esc(unavailableCopy.output)}</span>`;
   const primaryMarkup=`<section class="trace-primary"><div class="trace-primary-head"><h2>What went in and out</h2><span class="tiny">Primary trace content</span></div><div class="trace-primary-grid"><article class="trace-primary-card input"><h3>Input</h3><div class="trace-primary-value">${primaryInput}</div></article><article class="trace-primary-card output"><h3>Output</h3><div class="trace-primary-value">${primaryOutput}</div></article></div></section>`;
   ...
   $('tracePageBody').className='trace-page-body';$('tracePageBody').innerHTML=`<div class="human-shell">${header}${primaryMarkup}${presentationMarkup}${optionalMarkup}</div>`;
