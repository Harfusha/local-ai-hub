...
function tracePresentationColumns(title,left,right,budget,headerExtra='',leftTitle='Request details',rightTitle='Result details',aggregate=false){const leftMarkup=left.filter(Boolean).join(''),rightMarkup=right.filter(Boolean).join(''),groups=tracePresentationGroup(leftTitle,leftMarkup,'request')+tracePresentationGroup(rightTitle,rightMarkup,'result'),markup=`<section class="trace-primary trace-request-presentation"><div class="trace-primary-head"><h2>${esc(title)}</h2>${headerExtra}</div>${groups?`<div class="trace-primary-groups">${groups}</div>`:''}</section>`;return aggregate?traceBudgetMarkup(markup,traceRenderBudget()):traceFinalizeMarkup(markup,budget);}
...
function renderAsyncJobPresentation(model){
  const budget=traceRenderBudget(),safeModel=traceSanitizeValue(model||{}),lifecycle=safeModel.lifecycle||{},payload=traceSanitizeValue(tracePresentationPayload(safeModel,'asyncJob'));
  const stateText=traceInlineText(payload.status??lifecycle.state??lifecycle.status??'').trim().toLowerCase().replace(/\s+/g,'_'),terminalState=/^(complete|completed|success|succeeded|failed|error|cancelled|canceled|interrupted)$/.test(stateText),terminal=lifecycle.terminal===true||(lifecycle.terminal!==false&&terminalState),liveState=terminal?'finished':(/^(queued|pending|waiting|running|active|processing)$/.test(stateText)||lifecycle.terminal===false?'live / incomplete':'incomplete');
  const jobType=tracePresentationPick(payload,['job_type','jobType','type','kind'],safeModel.identity?.action||safeModel.session?.kind||'background job'),status=tracePresentationPick(payload,['status','state'],lifecycle.state??''),progress=tracePresentationPick(payload,['progress','progress_percent','percent','completed','current','total'],lifecycle.progress??''),queueWait=tracePresentationPick(payload,['queue_wait_ms','queue_wait','queue_time_ms'],''),retries=tracePresentationPick(payload,['retries','retry_count','attempts'],''),result=tracePresentationPick(payload,['result','output'],safeModel.output),error=tracePresentationPick(payload,['error'],safeModel.errors);
