...
<style>
:root{color-scheme:dark;font-family:Inter,ui-monospace,SFMono-Regular,Consolas,monospace;--bg:#0a0d11;--panel:#11171e;--panel2:#151d26;--line:#273341;--muted:#8594a7;--fg:#dbe5ef;--ok:#7ff19a;--warn:#ffd16b;--bad:#ff8585;--accent:#8fb7ff;--accent2:#b49bff}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--fg)}button{font:inherit}.top{position:sticky;top:0;z-index:20;background:#0e141bcc;border-bottom:1px solid var(--line);backdrop-filter:blur(10px)}.toolbar{display:flex;align-items:center;gap:9px;padding:10px 14px;flex-wrap:wrap}.brand{font-weight:700}.pill{border:1px solid #394758;border-radius:999px;padding:2px 7px;font-size:12px}.btn{background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:6px 10px;cursor:pointer}.btn:hover{background:#243343}.btn.bad{border-color:#774444;color:#ffb2b2}.btn.warn{border-color:#725d32;color:#ffe09b}.spacer{margin-left:auto}.muted,.tiny{color:var(--muted);font-size:11px}.ok{color:var(--ok)}.warn-t{color:var(--warn)}.bad-t{color:var(--bad)}.tabs{display:flex;gap:4px;padding:0 12px 8px;overflow:auto}.tabbtn{border:0;background:transparent;color:#9eacbc;padding:7px 10px;border-radius:6px;cursor:pointer;white-space:nowrap}.tabbtn.active{background:#1b2632;color:#fff}.page{display:none;padding:12px}.page.active{display:block}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:9px}.card,.section{background:var(--panel);border:1px solid var(--line);border-radius:9px}.card{padding:11px;min-height:90px}.label{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.05em}.value{font-size:20px;margin-top:6px;overflow-wrap:anywhere}.sub{color:#aab6c4;font-size:11px;margin-top:5px;line-height:1.45}.section{margin-top:11px;overflow:hidden;border-radius:9px}.section h2{font-size:13px;margin:0;padding:10px 12px;background:#131a22;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;gap:8px}.table-wrap{overflow:auto;max-height:520px}table{width:100%;border-collapse:collapse;font-size:11px}th,td{text-align:left;padding:7px 8px;border-bottom:1px solid #202a35;white-space:nowrap}th{position:sticky;top:0;background:#151d26;color:#95a4b7;z-index:2}tbody tr.click{cursor:pointer}tbody tr.click:hover{background:#151e27}.kv{display:grid;grid-template-columns:minmax(130px,220px) 1fr;gap:7px 12px;padding:10px 12px;font-size:12px}.kv>div:nth-child(odd){color:var(--muted)}.split{display:grid;grid-template-columns:1.2fr .8fr;gap:10px}.bar{height:5px;background:#283441;border-radius:4px;overflow:hidden;margin-top:6px}.bar>i{display:block;height:100%;background:#7898c7}.event{display:grid;grid-template-columns:82px 72px 110px 130px minmax(180px,1fr) 90px 85px;gap:7px;border-bottom:1px solid #202a35;padding:6px 8px;font-size:11px}.event:hover{background:#151e27}.events{max-height:650px;overflow:auto}.empty{padding:14px;color:var(--muted);font-size:12px}.chip{display:inline-block;border:1px solid #364555;border-radius:5px;padding:1px 5px;margin:1px 3px 1px 0}.mono{font-variant-numeric:tabular-nums}.modal-bg{display:none;position:fixed;inset:0;background:#000a;z-index:60;align-items:center;justify-content:center;padding:16px}.modal-bg.open{display:flex}.modal{width:min(900px,96vw);max-height:88vh;overflow:auto;background:#10161d;border:1px solid #344150;border-radius:10px;box-shadow:0 20px 80px #000;padding:0}.modal-head{position:sticky;top:0;background:#151d26;border-bottom:1px solid var(--line);display:flex;align-items:center;padding:10px 12px}.modal pre{white-space:pre-wrap;word-break:break-word;padding:12px;margin:0;color:#c9d6e4;font-size:12px}.danger-note{border-left:3px solid var(--warn);padding:8px 10px;background:#241f13;font-size:11px}.statline{display:flex;gap:12px;flex-wrap:wrap}.statline span{font-size:11px;color:#aab6c4}
...
.pulse-dot{width:6px;height:6px;border-radius:50%;background:#38bdf8;box-shadow:0 0 6px #38bdf8;animation:pulse 1.2s infinite}
...
.legend-strip{display:flex;flex-wrap:wrap;gap:8px 12px;padding:8px 12px;background:#131a22;border-bottom:1px solid var(--line);font-size:11px}
.legend-item{display:inline-flex;align-items:center;gap:4px;color:#94a3b8}
...
.control-menu{position:relative;margin-left:auto}.control-menu summary{list-style:none;cursor:pointer;background:#19232d;border:1px solid #394758;border-radius:6px;padding:6px 10px;font-size:11px}.control-menu summary::-webkit-details-marker{display:none}.control-menu[open] summary{background:#243343;color:#fff}.control-popover{position:absolute;right:0;top:calc(100% + 7px);z-index:30;min-width:330px;padding:10px;background:#10161d;border:1px solid #394758;border-radius:8px;box-shadow:0 16px 45px #000b;display:grid;grid-template-columns:1fr 1fr;gap:7px}.control-popover .token{grid-column:1/-1;display:flex;gap:6px}.control-popover .token input{min-width:0;flex:1;background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:6px 8px;font-size:11px}.control-popover button{width:100%}.project-toolbar{display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid var(--line);flex-wrap:wrap}.project-toolbar input,.project-toolbar select{background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:7px 9px;font-size:11px}.project-toolbar input{flex:1;min-width:180px}.project-toolbar select{min-width:118px}.project-summary{margin-left:auto}.project-table th:nth-child(1){width:25%}.project-table th:nth-child(2){width:33%}.project-table th:nth-child(3){width:22%}.project-table th:nth-child(4){width:12%}.project-table th:nth-child(5){width:8%}.project-table td{vertical-align:middle}.project-name{max-width:260px;overflow:hidden;text-overflow:ellipsis}.project-activity{white-space:normal;line-height:1.35;max-width:390px}.project-progress{min-width:150px}.project-progress-line{display:flex;justify-content:space-between;font-size:10px}.project-index{display:flex;gap:4px;flex-wrap:wrap}.project-index .chip{font-size:10px;margin:0}.project-actions{display:flex;gap:3px}.primary-metric{font-size:20px;font-weight:650}.sub strong{color:var(--fg)}
...
@media(max-width:700px){.toolbar{padding:8px;gap:6px}.toolbar .brand{font-size:12px}.toolbar>.tiny{display:none}.control-menu{margin-left:0}.control-popover{position:fixed;left:8px;right:8px;top:72px;min-width:0}.tabs{padding:0 6px 6px}.tabbtn{padding:6px 8px}.project-toolbar{align-items:stretch}.project-toolbar input{flex-basis:100%}.project-summary{margin-left:0;width:100%}.project-table th:nth-child(3),.project-table td:nth-child(3),.project-table th:nth-child(4),.project-table td:nth-child(4){display:none}.project-table th:nth-child(1){width:38%}.project-table th:nth-child(2){width:48%}.project-table th:nth-child(5){width:14%}.project-name{max-width:150px}.project-activity{max-width:220px}.project-progress{min-width:100px}.control-popover .token{flex-wrap:wrap}.control-popover .token input{flex-basis:100%}}
</style><style>.system-card .value{font-size:15px}.trace-running{color:var(--accent)}.trace-done{color:var(--ok)}.trace-failed{color:var(--bad)}.trace-queued{color:var(--warn)}.trace-toolbar{display:flex;gap:8px;align-items:center;padding:9px 12px;border-bottom:1px solid var(--line);flex-wrap:wrap}.trace-toolbar select{background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:5px 8px;font:inherit;font-size:11px}.human-shell{padding:12px}.human-summary{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:12px}.human-summary .badge{border:1px solid #394758;border-radius:999px;padding:3px 8px;font-size:11px}.human-summary .badge.ok{border-color:#16a34a}.human-summary .badge.bad{border-color:#b91c1c}.human-section{border:1px solid var(--line);border-radius:7px;margin:9px 0;overflow:hidden}.human-section>h3{font-size:12px;margin:0;padding:8px 10px;background:#151d26;border-bottom:1px solid var(--line)}.human-grid{display:grid;grid-template-columns:minmax(130px,220px) 1fr;gap:6px 12px;padding:10px;font-size:11px}.human-grid>div:nth-child(odd){color:var(--muted)}.human-value{white-space:pre-wrap;word-break:break-word;color:var(--fg)}.human-pre{margin:0;padding:10px;background:#0d1219;color:#c9d6e4;white-space:pre-wrap;word-break:break-word;font:inherit;line-height:1.45;max-height:360px;overflow:auto}.human-list{padding:8px 10px}.human-item{padding:7px 0;border-bottom:1px solid #202a35}.human-item:last-child{border-bottom:0}.event-card{padding:8px 10px;border-bottom:1px solid #202a35}.event-card:last-child{border-bottom:0}.event-head{display:flex;gap:9px;align-items:center;margin-bottom:5px}.event-type{color:var(--accent);font-weight:600}.event-time{color:var(--muted);font-size:10px}.trace-timeline{padding:10px}.trace-step{border-left:2px solid #30445e;margin-left:9px;padding-left:12px;padding-bottom:10px}.trace-step:last-child{padding-bottom:0}.trace-step-head{display:flex;align-items:center;gap:8px;width:100%;text-align:left;background:#151d26;color:var(--fg);border:1px solid #334355;border-radius:6px;padding:8px 10px;cursor:pointer;font:inherit}.trace-step-head:hover{background:#1b2a38}.trace-step-index{color:var(--accent);font-weight:700}.trace-step-chevron{margin-left:auto;color:var(--muted);transition:transform .15s}.trace-step.open .trace-step-chevron{transform:rotate(90deg)}.trace-step-body{display:none;margin-top:5px;border:1px solid #273544;border-radius:6px;overflow:hidden}.trace-step.open .trace-step-body{display:block}.trace-event{padding:8px 10px;border-bottom:1px solid #202a35}.trace-event:last-child{border-bottom:0}.trace-output{color:#c9d6e4;white-space:pre-wrap;word-break:break-word;background:#0d1219;padding:8px;max-height:260px;overflow:auto}.trace-stream-summary{background:#0d1219;border:1px solid #263546;border-radius:6px;padding:8px}.trace-stream-summary strong{display:block;color:var(--fg);font-size:11px;margin-bottom:6px}.trace-stream-summary .trace-output{padding:0;border:0;max-height:180px}.raw-json{margin-top:10px;border:1px solid #303c4a;border-radius:6px}.raw-json summary{cursor:pointer;padding:8px 10px;color:var(--muted);font-size:11px}.raw-json pre{max-height:320px}.empty-human{padding:12px;color:var(--muted);font-size:11px}</style></head><body>
 <div class="top"><div class="toolbar"><span class="brand">Local AI Hub — realtime</span><span id="conn" class="pill warn-t">connecting</span><span id="updated" class="tiny"></span><label class="tiny">SLO <select id="sloScope"><option value="process" selected>Since restart</option><option value="window">Last 30 days</option></select></label><span class="tiny spacer">debug traces · bounded prompt/output retention</span><details id="controlMenu" class="control-menu"><summary>Controls</summary><div class="control-popover"><div class="token"><input type="password" id="apiToken" autocomplete="off" placeholder="API token (remote only)"><button class="btn" id="saveToken">Set token</button></div><button class="btn ok" id="optDbBtn">Optimize DBs</button><button class="btn warn" id="purgeCacheBtn">Purge Cache</button><button class="btn" id="doctorBtn">Doctor</button><button class="btn" id="pauseEvents">Pause events</button><button class="btn warn" id="prepToggle">Pause preprocessing</button><button class="btn" id="restartHub">Restart hub</button><button class="btn bad" id="stopService">Stop service</button></div></details></div>
<div class="tabs"><button class="tabbtn active" data-tab="overview">Overview</button><button class="tabbtn" data-tab="work">Queue & requests</button><button class="tabbtn" data-tab="projects">Projects</button><button class="tabbtn" data-tab="performance">Models & cache</button><button class="tabbtn" data-tab="reliability">Reliability</button><button class="tabbtn" data-tab="config">Configuration</button><button class="tabbtn" data-tab="events">Live events</button><button class="tabbtn" data-tab="bundles">Bundles</button></div></div>

<div id="overview" class="page active"><div class="grid">
...
<div class="card"><div class="label">Queue</div><div class="value primary-metric" id="fgQueue">…</div><div class="sub" id="bgQueue"></div></div>
<div class="card"><div class="label" id="handledRequestsLabel">Requests handled · since restart</div><div class="value primary-metric" id="handledRequests">…</div><div class="sub" id="handledRequestsSub"></div></div>
...
<div class="card"><div class="label">Preprocessing</div><div class="value primary-metric" id="prep">…</div><div class="sub" id="prepSub"></div></div>
<div class="card"><div class="label">Latency p50 / p95 / p99</div><div class="value" id="latency">…</div><div class="sub" id="queueWait"></div></div>
...
<div class="card system-card"><div class="label">System</div><div class="value" id="sysUtil">…</div><div class="sub" id="sysSub"></div></div>
</div>
<div class="split"><section class="section"><h2>Current workload <span class="tiny" id="currentSummary"></span></h2><div class="table-wrap"><table><thead><tr><th>State</th><th>Agent/tenant</th><th>Source</th><th>Model</th><th>Wait</th><th>Processing</th><th>Reason</th></tr></thead><tbody id="overviewJobs"></tbody></table></div></section><section class="section"><h2>Hotspots</h2><div id="hotspots" class="kv"></div></section></div></div>

<div id="work" class="page"><section class="section"><h2>Scheduler queue <span class="tiny" id="queueSummary"></span></h2><div class="table-wrap"><table><thead><tr><th>State</th><th>Job</th><th>Agent/tenant</th><th>Source</th><th>Model</th><th>Priority</th><th>Wait</th><th>Processing</th><th>Reason</th></tr></thead><tbody id="jobs"></tbody></table></div></section><section class="section"><h2>Active API requests <span class="tiny">monitoring endpoints excluded</span></h2><div class="table-wrap"><table><thead><tr><th>Request</th><th>Agent</th><th>Tenant</th><th>Action</th><th>Age</th></tr></thead><tbody id="activeReq"></tbody></table></div></section><section class="section"><h2>Recent API requests <span class="tiny">persisted telemetry · monitoring endpoints excluded</span></h2><div class="table-wrap"><table><thead><tr><th>Time</th><th>Request</th><th>Agent</th><th>Tenant</th><th>Action</th><th>Status</th><th>Duration</th></tr></thead><tbody id="recentReq"></tbody></table></div></section><section class="section"><h2>Agent debug traces <span class="tiny" id="traceSummary">full prompt/output · bounded retention</span></h2><div class="trace-toolbar"><span class="tiny">Click a trace for live events, model prompt, tool calls and output.</span><select id="traceKind"><option value="">All kinds</option><option value="api_request">API requests</option><option value="async_job">Async jobs</option></select><button class="btn" id="traceRefresh" style="padding:4px 8px;font-size:11px">Refresh</button></div><div class="table-wrap"><table><thead><tr><th>State</th><th>Kind</th><th>Action</th><th>Agent / tenant</th><th>Model</th><th>Created</th><th>Updated</th><th>Links</th></tr></thead><tbody id="traces"></tbody></table></div></section></div>

<div id="projects" class="page"><div id="prepDiagnosticBar" class="diag-banner info" style="display:none"></div><section class="section"><h2><span style="display:flex;align-items:center;gap:8px">Projects <span class="tiny" id="prepState"></span></span><div style="display:flex;gap:6px;align-items:center"><button class="btn warn" id="prepAllToggle" style="padding:4px 10px;font-size:11px">Pause all</button><button class="btn ok" id="regProjectBtn" style="padding:4px 10px;font-size:11px">+ Register</button><button class="btn warn" id="cleanMissingBtn" style="padding:4px 10px;font-size:11px">🧹 Clean missing</button></div></h2><div class="project-toolbar"><input id="projectSearch" type="search" placeholder="Search projects…" autocomplete="off"><select id="projectFilter" aria-label="Project status"><option value="all">All states</option><option value="running">Running</option><option value="waiting">Waiting</option><option value="error">Error</option><option value="paused">Paused</option><option value="ready">Ready</option></select><select id="projectSort" aria-label="Project sort"><option value="priority">Operational priority</option><option value="name">Name</option><option value="progress">Progress</option><option value="recent">Recent activity</option></select><span class="tiny project-summary" id="projectSummary"></span></div><div class="table-wrap"><table class="project-table"><thead><tr><th>Project</th><th>State &amp; activity</th><th>Phase &amp; progress</th><th>Indexes</th><th>Actions</th></tr></thead><tbody id="projectsBody"></tbody></table></div></section></div>

<div id="explorer" class="page"><section class="section"><h2>Code &amp; AST Symbol Explorer <span class="tiny">Search any class, function or method across indexed projects</span></h2><div style="padding:12px;display:flex;gap:8px"><input type="text" id="codeSearchInput" placeholder="Symbol name (e.g. PlayerController, CalculateTax, Health)..." style="flex:1;background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:8px 12px;font-size:12px"><button class="btn ok" id="codeSearchBtn">Inspect Symbol</button></div><div id="symbolDetails" style="padding:12px;display:none"><div style="display:flex;gap:12px;margin-bottom:8px;font-size:12px;flex-wrap:wrap"><span><b>Symbol:</b> <span id="symName" class="chip"></span></span><span><b>Kind:</b> <span id="symKind" class="chip"></span></span><span><b>File:</b> <span id="symPath" class="muted"></span></span><span><b>Lines:</b> <span id="symLines" class="mono"></span></span></div><div style="display:flex;gap:8px;margin-bottom:8px"><button class="btn" id="genTestsBtn">Generate Unit Tests</button><button class="btn warn" id="impactCheckBtn">Analyze Refactor Impact</button><button class="btn ok" id="resolveImpBtn">Resolve Imports</button></div><pre id="symCode" style="background:#0d1219;padding:12px;border-radius:6px;overflow:auto;max-height:480px;font-size:12px;border:1px solid #273341;color:#dbe5ef"></pre></div></section></div>

<div id="performance" class="page"><section class="section"><h2>Execution profiles <span class="tiny">configured capacity; context packing remains adaptive</span></h2><div class="table-wrap"><table><thead><tr><th>Model</th><th>Tier</th><th>Context</th><th>Max</th><th>Parallel</th><th>Thinking</th><th>Prompt cap</th></tr></thead><tbody id="executionProfiles"></tbody></table></div></section><div class="split"><section class="section"><h2>Model statistics</h2><div class="table-wrap"><table><thead><tr><th>Model</th><th>Calls</th><th>Avg</th><th>Load</th><th>Fails</th></tr></thead><tbody id="models"></tbody></table></div></section><section class="section"><h2>Cache layers</h2><div class="table-wrap"><table><thead><tr><th>Layer</th><th>Calls</th><th>Avg</th><th>Tokens saved</th></tr></thead><tbody id="cacheLayers"></tbody></table></div></section></div><div class="split"><section class="section"><h2>Agents</h2><div class="table-wrap"><table><thead><tr><th>Agent</th><th>Requests</th><th>Local AI</th><th>Avg</th><th>Fails</th></tr></thead><tbody id="agents"></tbody></table></div></section><section class="section"><h2>Execution routes</h2><div class="table-wrap"><table><thead><tr><th>Route</th><th>Task</th><th>Complexity</th><th>Calls</th><th>Avg</th><th>Fails</th></tr></thead><tbody id="routes"></tbody></table></div></section></div></div>

<div id="commands" class="page"><section class="section"><h2>Safe command runner <span class="tiny">same fail-closed policy broker used by agents</span></h2><div style="padding:12px;display:grid;grid-template-columns:minmax(180px,.7fr) minmax(280px,2fr) auto auto;gap:8px"><input id="cmdRoot" placeholder="Repository root" style="background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:8px"><input id="cmdInput" placeholder="pytest -q / npm test / cargo check …" style="background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:8px"><button class="btn" id="cmdClassify">Classify</button><button class="btn ok" id="cmdRun">Run allowed command</button></div><pre id="cmdOutput" style="margin:0;padding:12px;white-space:pre-wrap;max-height:360px;overflow:auto;background:#0d1219;color:#c9d6e4;font-size:11px"></pre></section><section class="section"><h2>Running commands <span class="tiny" id="commandState"></span></h2><div class="table-wrap"><table><thead><tr><th>Command</th><th>CWD</th><th>Tenant</th><th>Class</th><th>Age</th><th>Timeout</th></tr></thead><tbody id="activeCommands"></tbody></table></div></section><div class="split"><section class="section"><h2>Command broker statistics</h2><div id="commandStats" class="kv"></div></section><section class="section"><h2>Blocked by policy</h2><div class="table-wrap"><table><thead><tr><th>Reason</th><th>Count</th></tr></thead><tbody id="blockedReasons"></tbody></table></div></section></div></div>


<div id="config" class="page"><section class="section"><h2>Runtime configuration <span class="tiny">safe override sidecar · restart required after save</span></h2><div style="padding:12px"><div class="kv"><div>Hardware profile</div><div><select id="cfgProfile" style="background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:6px"><option>auto</option><option>cpu</option><option>low</option><option>balanced</option><option>high</option><option>max</option></select></div><div>Preprocessing</div><div><input type="checkbox" id="cfgPreprocess"> enabled</div><div>Managed code intelligence</div><div><input type="checkbox" id="cfgIntel"> enabled</div><div>Serena</div><div><input type="checkbox" id="cfgSerena"> enabled</div><div>CodeGraphContext</div><div><input type="checkbox" id="cfgCodegraph"> enabled</div></div><div style="display:flex;gap:8px;flex-wrap:wrap;padding-top:10px"><button class="btn ok" id="cfgSave">Save overrides</button><button class="btn" id="cfgReload">Reload view</button><button class="btn warn" id="cfgReset">Reset dashboard overrides</button><span class="tiny" id="cfgStatus"></span></div></div></section><section class="section"><h2>Effective configuration <span class="tiny" id="cfgPath"></span></h2><pre id="cfgPreview" style="margin:0;padding:12px;white-space:pre-wrap;max-height:520px;overflow:auto;background:#0d1219;color:#c9d6e4;font-size:11px"></pre></section></div>

<div id="reliability" class="page"><div class="split"><section class="section"><h2>Recent error fingerprints</h2><div class="table-wrap"><table><thead><tr><th>Component</th><th>Operation</th><th>Count</th><th>Recovered</th><th>Last seen</th></tr></thead><tbody id="errors"></tbody></table></div></section><section class="section"><h2>Runtime / scheduler counters</h2><div id="runtimeCounters" class="kv"></div></section></div><section class="section"><h2>Code-intelligence process control <span class="tiny">bounded Serena / CodeGraph sessions</span></h2><div style="padding:12px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn" id="intelRediscover">Rediscover executables</button><button class="btn warn" id="intelReset">Reset all sessions</button><span class="tiny" id="intelControlStatus"></span></div></section><section class="section"><h2>Operational log tail <button class="btn" id="loadLogs" style="padding:3px 7px">Refresh</button></h2><pre id="logTail" style="margin:0;padding:12px;white-space:pre-wrap;max-height:420px;overflow:auto;background:#0d1219;color:#c9d6e4;font-size:11px">Load on demand.</pre></section></div>
...
<div id="architecture" class="page"><section class="section"><h2>Project Dependency Graph <span class="tiny" id="archSummary"></span></h2><div style="padding:10px;display:flex;gap:8px;flex-wrap:wrap;align-items:center"><button class="btn" id="archRefresh">Refresh projects</button><input type="text" id="symbolInput" placeholder="Symbol name (e.g. PlayerController)..." style="background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:6px 10px;font-size:11px"><button class="btn" id="symbolGraphBtn">View Call-Graph</button><button class="btn warn" id="deadCodeBtn">Scan Dead Code</button><button class="btn ok" id="auditDepsBtn">Audit Security</button><span class="tiny muted">Drag nodes to rearrange · Click symbol nodes for details</span></div><svg id="archSvg" style="width:100%;height:520px;background:#0d1219;border-radius:6px;display:block"></svg><div class="table-wrap" style="margin-top:10px"><table><thead><tr><th>From</th><th>To</th><th>Type</th><th>Shared label / Reference</th></tr></thead><tbody id="archEdgesTable"></tbody></table></div></section><section class="section" id="deadCodeSec" style="display:none"><h2>Detected Dead Code &amp; Unused Symbols <span class="tiny" id="deadCodeSummary"></span></h2><div class="table-wrap"><table><thead><tr><th>Symbol</th><th>Kind</th><th>File</th><th>Line</th><th>Container</th><th>Reason</th></tr></thead><tbody id="deadCodeTable"></tbody></table></div></section><section class="section" id="auditSec" style="display:none"><h2>Dependency Vulnerability &amp; CVE Audit <span class="tiny" id="auditSummary"></span></h2><div class="table-wrap"><table><thead><tr><th>Package</th><th>Severity</th><th>Installed</th><th>Safe Version</th><th>Advisory</th><th>Manifest</th></tr></thead><tbody id="auditTable"></tbody></table></div></section></div>

<div id="bundles" class="page"><section class="section"><h2>Project Bundles — Export &amp; Import</h2><div style="padding:12px"><p class="tiny muted">Bundles compress preprocessed index (AST, deterministic facts, RAG vectors and semantic cards) for instant restore on a different machine without re-indexing.</p><div class="split" style="margin-top:10px"><div><h3 style="font-size:12px;margin:0 0 8px">Export Bundle</h3><table><thead><tr><th>Project</th><th>Status</th><th>Action</th></tr></thead><tbody id="bundleExportTable"></tbody></table></div><div><h3 style="font-size:12px;margin:0 0 8px">Import Bundle</h3><div style="display:flex;flex-direction:column;gap:8px"><input type="file" id="bundleFile" accept=".zip,application/zip" style="color:var(--fg);font-size:12px"><input type="text" id="bundleTargetRoot" placeholder="Optional target repository root" style="background:#19232d;color:var(--fg);border:1px solid #394758;border-radius:6px;padding:7px;color:var(--fg)"><button class="btn" id="bundleImport">Import selected ZIP</button><div id="bundleImportStatus" class="tiny muted"></div></div></div></div></div></section></div>
...
let sloScope=$('sloScope').value;$('sloScope').onchange=()=>{sloScope=$('sloScope').value;pollStatus()};
$('saveToken').onclick=()=>{apiToken=$('apiToken').value.trim();pollStatus()};
async function apiFetch(path,opts={}){opts={...opts};const h=new Headers(opts.headers||{});if(apiToken)h.set('X-LocalAI-Token',apiToken);opts.headers=h;let r=await nativeFetch(path,opts);if(r.status===401&&!apiToken){const entered=prompt('Local AI Hub API token');if(entered){apiToken=entered.trim();$('apiToken').value=apiToken;try{sessionStorage.setItem('localAiHubToken',apiToken)}catch{};h.set('X-LocalAI-Token',apiToken);r=await nativeFetch(path,{...opts,headers:h})}}return r}
...
const dataStore=new Map(); let seq=0; function clickableRow(obj,html){const id='d'+(++seq);dataStore.set(id,obj);return `<tr class="click" data-detail="${id}">${html}</tr>`}
  ...
  function renderTraceDetail(d){activeTraceData=d;const s=d?.session||{},events=d?.events||[],payload={session:{trace_id:s.trace_id,kind:s.kind,state:s.state,tenant:s.tenant,agent:s.agent,action:s.action,source:s.source,model:s.model,request_id:s.request_id,async_job_id:s.async_job_id,scheduler_job_id:s.scheduler_job_id,created_at:s.created_at,updated_at:s.updated_at,finished_at:s.finished_at,error:s.error,text_bytes:s.text_bytes},main_agent_prompt:s.effective_payload||{},original_request:s.request||{},output:s.output||'',response:s.response||{},events};$('tracePageTitle').textContent=String(s.action||s.source||'Agent trace');$('tracePageLive').textContent=d?.terminal?'terminal · retained':'● live · auto-refresh';$('tracePageLive').className='tiny '+(d?.terminal?'ok':'trace-running');const state=traceDisplayState(s),displayState=traceStatus(s),stateClass=(displayState==='failed'||displayState==='error')?'bad':(displayState==='interrupted'?'warn':(d?.terminal?'ok':'warn')),header=`<div class="trace-inspector-head"><div><div class="trace-kicker">Agent execution</div><strong>${esc(s.action||s.source||'Trace')}</strong><div class="tiny">${esc(s.agent||'unknown agent')} · ${esc(s.model||'model not recorded')}</div></div><span class="badge ${stateClass}">${esc(state)}</span></div><div class="trace-metrics"><span>${events.length} events</span><span>${esc(s.tenant||'no tenant')}</span><span>${s.text_bytes||0} bytes retained</span></div>`,timeline=`<section class="human-section"><h3>Agent timeline (${events.length})</h3>${traceTimeline(events)}</section>`,prompt=renderModelPrompt(payload.main_agent_prompt)+humanSection('Original request',payload.original_request),request=humanSection('Original request',payload.original_request),output=humanSection('Output',payload.output),response=humanSection('Final response',payload.response),views={timeline, prompt:prompt, output:output+response, events:`<section class="human-section"><h3>All events</h3>${renderAny(events)}</section>`, raw:traceRaw(payload)},content=views[traceView]||timeline;$('tracePageBody').className='trace-page-body';$('tracePageBody').innerHTML=`<div class="human-shell">${header}<nav class="trace-tabs" aria-label="Trace views">${traceTab('Timeline','timeline')}${traceTab('Prompt','prompt')}${traceTab('Output','output')}${traceTab('Events','events')}${traceTab('Raw','raw')}</nav><div class="trace-view">${content}</div></div>`}
  ...
  async function openTrace(id){activeTraceId=String(id||'');traceSeq=0;traceEvents=[];traceOpenSteps=new Set();traceView='timeline';activeTraceData=null;renderTraceList(lastTraces);switchTab('traceInspector');if(traceTimer)clearInterval(traceTimer);const poll=async()=>{if(!activeTraceId)return;try{const r=await apiFetch('/v1/debug-traces/'+encodeURIComponent(activeTraceId)+'?since_seq='+traceSeq,{cache:'no-store'}),d=await r.json();if(d.success){traceSeq=Number(d.next_seq||traceSeq);traceEvents=traceEvents.concat(d.events||[]);renderTraceDetail({...d,events:traceEvents});if(d.terminal){clearInterval(traceTimer);traceTimer=null}}else{openModal(d,'Trace error');clearInterval(traceTimer);traceTimer=null}}catch(e){$('tracePageLive').textContent='refresh failed: '+e.message}};await poll();traceTimer=setInterval(poll,1000)}
  ...
  $('modalClose').onclick=()=>{$('modalBg').classList.remove('open');activeTraceId='';activeTraceData=null;if(traceTimer)clearInterval(traceTimer);traceTimer=null};$('modalBg').onclick=e=>{if(e.target===$('modalBg'))$('modalClose').click()};document.addEventListener('click',e=>{const tab=e.target.closest?.('[data-trace-view]');if(tab){setTraceView(tab.dataset.traceView);return}const pageTrace=e.target.closest?.('[data-trace-page-id]');if(pageTrace){openTrace(pageTrace.dataset.tracePageId);return}const back=e.target.closest?.('[data-trace-back]');if(back){switchTab('work');return}const toggle=e.target.closest?.('[data-trace-toggle]');if(toggle){toggleTraceStep(toggle);return}if(e.target.closest('button, a, input, select, textarea, .action-btn-sm, [data-project-action], [data-export-root]'))return;const trace=e.target.closest?.('[data-trace-id]');if(trace){openTrace(trace.dataset.traceId);return}const tr=e.target.closest?.('[data-detail]');if(tr)openModal(dataStore.get(tr.dataset.detail)||{},'Details')});
...
async function loadConfigView(){try{const r=await apiFetch('/v1/config',{cache:'no-store'}),d=await r.json();if(!d.success){$('cfgStatus').textContent=d.error||'failed';return}const c=d.config||{};$('cfgProfile').value=c.hardware?.profile||'auto';$('cfgPreprocess').checked=!!c.preprocessing?.enabled;$('cfgIntel').checked=!!c.code_intelligence?.enabled;$('cfgSerena').checked=!!c.code_intelligence?.serena_enabled;$('cfgCodegraph').checked=!!c.code_intelligence?.codegraph_enabled;$('cfgPath').textContent=(d.config_path||'')+(d.runtime_override_path?' · overrides '+d.runtime_override_path:'');$('cfgPreview').textContent=JSON.stringify(c,null,2);$('cfgStatus').textContent=''}catch(e){$('cfgStatus').textContent=String(e)}}
$('cfgReload').onclick=loadConfigView;
$('cfgSave').onclick=async()=>{const settings={'hardware.profile':$('cfgProfile').value,'preprocessing.enabled':$('cfgPreprocess').checked,'code_intelligence.enabled':$('cfgIntel').checked,'code_intelligence.serena_enabled':$('cfgSerena').checked,'code_intelligence.codegraph_enabled':$('cfgCodegraph').checked};const r=await post('/v1/config/update',{action:'update',settings});$('cfgStatus').textContent=r.success?'saved · restart hub to apply':('error: '+(r.error||'failed'));if(r.success)loadConfigView()};
$('cfgReset').onclick=async()=>{if(!confirm('Reset dashboard-managed configuration overrides?'))return;const r=await post('/v1/config/update',{action:'reset'});$('cfgStatus').textContent=r.success?'overrides reset · restart hub to apply':('error: '+(r.error||'failed'));if(r.success)loadConfigView()};
...
$('cmdRun').onclick=async()=>{const command=$('cmdInput').value.trim(),cwd=$('cmdRoot').value.trim();if(!command||!cwd){$('cmdOutput').textContent='Repository root and command are required.';return}const c=await post('/v1/command',{action:'classify',command});if(!c?.classification?.allowed){$('cmdOutput').textContent=JSON.stringify(c,null,2);return}if(!confirm('Run this '+c.classification.class+' command?\n\n'+command))return;$('cmdRun').disabled=true;try{$('cmdOutput').textContent=JSON.stringify(await post('/v1/command',{action:'run',command,cwd,force:true}),null,2)}finally{$('cmdRun').disabled=false;pollStatus()}};
...
$('loadLogs').onclick=async()=>{try{const r=await apiFetch('/v1/logs/tail?lines=250',{cache:'no-store'}),d=await r.json();$('logTail').textContent=d.success?(d.lines||[]).join('\n'):JSON.stringify(d,null,2)}catch(e){$('logTail').textContent=String(e)}};
...
$('regProjectBtn').onclick = () => {
  const path = prompt('Enter absolute path of project or repository to register (e.g. C:\\Projects\\MyRepo):');
...
$('codeSearchBtn').onclick=async()=>{const sym=$('codeSearchInput').value.trim();if(!sym)return;try{const r=await(await apiFetch('/v1/code/symbol?symbol='+encodeURIComponent(sym))).json();if(r.success){$('symbolDetails').style.display='block';$('symName').textContent=r.name;$('symKind').textContent=r.kind;$('symPath').textContent=r.path;$('symLines').textContent=r.line+'-'+r.end_line;$('symCode').textContent=r.snippet}else{openModal(r,'Symbol Search')}}catch(e){openModal({error:String(e)},'Error')}};
...
function render(s){last=s;dataStore.clear();seq=0;const q=s.scheduler||{},o=s.observability||{},p=s.preprocessing||{},bg=s.background_gpu||{},h=s.headless||{},r=s.runtime_stats||{},ss=q.stats||{},rp=s.runtime_profile||{},cmd=r.commands||{};
 ensureHttpTailTable();setupWorkLayout();
 const scopeLabel=sloScope==='process'?'since restart':'30d';$('handledRequestsLabel').textContent='Requests handled · '+scopeLabel;$('tokensSavedLabel').textContent='Tokens saved · '+scopeLabel;$('dollarsSavedLabel').textContent='Estimated savings · '+scopeLabel;$('cacheLabel').textContent='Cache hit rate · '+scopeLabel;$('reliabilityLabel').textContent='Reliability · '+scopeLabel;
 const firstProject=(p.projects||[]).find(x=>x.root);if(firstProject&&!$('cmdRoot').value)$('cmdRoot').value=firstProject.root;
  $('health').innerHTML=`<span class="${s.hub_online&&s.ollama_online?'ok':s.hub_online?'warn-t':'bad-t'}">${s.hub_online?'hub ✓':'hub ✗'} / ${s.ollama_online?'ollama ✓':'ollama ✗'} / ${esc(h.state||'n/a')}</span>`;$('uptime').textContent='uptime '+durSec(s.uptime_seconds)+' · supervisor restarts '+n(h.restarts);
  const ep=(rp.execution||{}).models||[]; // Execution profiles remain available on the detail tabs.
 const httpTail=o.http_tail_latency||{};rows('httpTail',httpTail.by_action||[],x=>clickableRow(x,`<td>${esc(x.action)}</td><td>${n(x.events)}</td><td>${ms(x.p50_duration_ms)}</td><td>${ms(x.p95_duration_ms)}</td><td>${ms(x.p99_duration_ms)}</td><td>${n(x.failures)}</td>`),6);
  $('fgQueue').textContent=n(q.foreground_queued)+' queued / '+n(q.foreground_inflight)+' running';$('bgQueue').textContent='background '+n(q.background_queued)+' queued / '+n(q.inflight_background)+' running · '+(q.background_allowed?'idle work allowed':'yielding');
  ...
  $('handledRequests').textContent=n(agentHttp.events);$('handledRequestsSub').textContent=n(agentHttp.failures)+' operational failures · '+n(policy.events)+' policy rejections';
  $('tokensSaved').textContent=n(savedTokens);$('tokensSavedSub').textContent='avoided cloud tokens · '+n(o.cache_hits)+' cache hits';
  $('dollarsSaved').textContent='≈ $'+savedUsd.toFixed(2);$('dollarsSavedSub').textContent='estimate · $'+rate.toFixed(2)+' / 1M avoided tokens';
  $('cache').textContent=((o.cache_hit_rate||0)*100).toFixed(1)+'%';$('cacheSub').textContent=n(o.cache_hits)+' hits · '+n(o.inference_events)+' local inference events';
 $('latency').textContent=ms(agentHttp.p50_duration_ms)+' / '+ms(agentHttp.p95_duration_ms)+' / '+ms(agentHttp.p99_duration_ms);$('queueWait').textContent='local inference p95 '+ms(inference.p95_duration_ms)+' · avg queue wait '+ms(o.avg_queue_wait_ms);
 $('reliabilityValue').innerHTML=`<span class="${agentHttp.failures?'warn-t':'ok'}">${n(agentHttp.failures)} failures</span>`;$('reliabilitySub').textContent=n(policy.events)+' policy rejections · '+n(inference.fallback_count)+' fallbacks · '+n(inference.degraded_count)+' degraded · '+n(inference.retry_count)+' retries';
 $('prep').textContent=(p.paused?'paused':(p.processing_projects?'active':'idle'))+' · '+n(p.steps)+' steps';$('prepSub').textContent=n(p.active_projects)+' active · '+n(p.processing_projects)+'/'+n(p.max_preprocessing_projects)+' preprocessing · '+n(p.waiting_projects)+' waiting';
  const allJobs=[...(q.inflight_jobs||[]),...(q.pending_jobs||[])];$('currentSummary').textContent=n(q.foreground_queued)+' queued · '+n(q.foreground_inflight)+' running'; rows('overviewJobs',allJobs.slice(0,12),j=>schedulerRow(j,`<td>${esc(j.state)}</td><td>${esc(j.tenant)}</td><td>${esc(j.source)}</td><td>${esc(j.model)}</td><td>${ms(j.wait_ms)}</td><td>${ms(j.service_ms)}</td><td>${esc(j.wait_reason)}</td>`),7);
 const hs=o.hotspots||[];$('hotspots').innerHTML=hs.length?hs.map((x,i)=>`<div>${esc(x.type||'signal')}</div><div><span class="chip">${esc(x.signal||'')}</span> ${esc(x.value??x.value_ms??x.count??'')}</div>`).join(''):'<div>Status</div><div class="ok">No persistent hotspot detected</div>';
  const visibleJobs=workVisible(allJobs),visibleActive=workVisible(o.active_requests||[]),visibleRecent=workVisible(o.recent_http||[]);$('queueSummary').textContent=n(q.foreground_queued)+' fg + '+n(q.background_queued)+' bg queued · '+n(q.foreground_inflight)+' fg + '+n(q.inflight_background)+' bg running';$('workQueued').textContent=n((q.foreground_queued||0)+(q.background_queued||0));$('workRunning').textContent=n((q.foreground_inflight||0)+(q.inflight_background||0));$('workActive').textContent=n((o.active_requests||[]).length);$('workRetained').textContent=n(lastTraces.length);$('workFilterSummary').textContent=`showing ${visibleJobs.length+visibleActive.length+visibleRecent.length} live/history items`;rows('jobs',visibleJobs,j=>workSchedulerRow(j),5);
 ...
 const prepPaused=!!p.paused;$('prepState').textContent=n((p.projects||[]).length)+' registered projects · global '+(prepPaused?'paused':'running');$('prepToggle').textContent=prepPaused?'Resume preprocessing':'Pause preprocessing';$('prepAllToggle').textContent=prepPaused?'Resume all projects':'Pause all projects';
 renderProjects(p.projects||[]);
 rows('executionProfiles',ep,x=>clickableRow(x,`<td>${esc(x.model)}</td><td>${esc(x.tier)}</td><td>${n(x.num_ctx)}</td><td>${n(x.max_ctx)}</td><td>${n(x.parallel_limit)}</td><td>${x.think?'on':'role-gated/off'}</td><td>${n(x.prompt_budget_tokens)}</td>`),7);rows('models',o.by_model||[],x=>clickableRow(x,`<td>${esc(x.model)}</td><td>${n(x.calls)}</td><td>${ms(x.avg_ms)}</td><td>${ms(x.avg_load_ms)}</td><td>${n(x.failures)}</td>`),5);rows('cacheLayers',o.cache_layers||[],x=>clickableRow(x,`<td>${esc(x.layer)}</td><td>${n(x.calls)}</td><td>${ms(x.avg_ms)}</td><td>${n(x.cloud_tokens_avoided_est)}</td>`),4);rows('agents',o.by_agent||[],x=>clickableRow(x,`<td>${esc(x.agent)}</td><td>${n(x.requests)}</td><td>${n(x.local_inference_calls)}</td><td>${ms(x.avg_ms)}</td><td>${n(x.failures)}</td>`),5);rows('routes',o.execution_routes||[],x=>clickableRow(x,`<td>${esc(x.route)}</td><td>${esc(x.task_type)}</td><td>${esc(x.complexity)}</td><td>${n(x.calls)}</td><td>${ms(x.avg_ms)}</td><td>${n(x.failures)}</td>`),6);
 $('commandState').textContent=n(cmd.active_count)+' running';rows('activeCommands',cmd.active||[],x=>clickableRow(x,`<td>${esc(x.command)}</td><td>${esc(x.cwd)}</td><td>${esc(x.tenant)}</td><td>${esc(x.class)}</td><td>${age(x.age_ms)}</td><td>${durSec(x.timeout_seconds)}</td>`),6);$('commandStats').innerHTML=`<div>Executed</div><div>${n(cmd.executed)}</div><div>Cache hits / misses</div><div>${n(cmd.hits)} / ${n(cmd.misses)}</div><div>Coalesced waiters</div><div>${n(cmd.coalesced_waiters)}</div><div>Policy blocked</div><div>${n(cmd.blocked)}</div>`;rows('blockedReasons',Object.entries(cmd.blocked_by_reason||{}),x=>clickableRow({reason:x[0],count:x[1]},`<td>${esc(x[0])}</td><td>${n(x[1])}</td>`),2);
 rows('errors',o.recent_errors||[],x=>clickableRow(x,`<td>${esc(x.component)}</td><td>${esc(x.operation)}</td><td>${n(x.count)}</td><td>${n(x.recovered_count)}</td><td>${x.last_seen?new Date(x.last_seen*1000).toLocaleTimeString():'—'}</td>`),5);$('runtimeCounters').innerHTML=`<div>Scheduler submitted</div><div>${n(ss.submitted)}</div><div>Completed / failed</div><div>${n(ss.completed)} / ${n(ss.failed)}</div><div>Model switches</div><div>${n(ss.model_switches)}</div><div>Queue rejections</div><div>${n(ss.queue_rejections)}</div><div>Caller timeouts</div><div>${n(ss.caller_timeouts)}</div><div>Background yields</div><div>${n(ss.background_yields)}</div><div>Supervisor restarts</div><div>${n(h.restarts)}</div>`;
 renderBundles(s);
}
...
async function pollEvents(){try{const r=await apiFetch('/v1/live?after='+cursor+'&limit=200',{cache:'no-store'}),d=await r.json();cursor=Number(d.cursor||cursor);renderEvents(d.events||[])}catch{}}
 probeHealth();pollStatus();pollEvents();pollTraces();setInterval(probeHealth,5000);setInterval(pollStatus,3000);setInterval(pollEvents,1000);setInterval(pollTraces,2000);
// ── Architecture Graph ────────────────────────────────────────────────────────
...
document.querySelectorAll('.tabbtn').forEach(b=>b.addEventListener('click',()=>{if(b.dataset.tab==='architecture'&&!archData)loadArchGraph()}));
// ── Bundles ────────────────────────────────────────────────────────────────────
...
document.addEventListener('click',event=>{const button=event.target.closest('[data-project-action],[data-export-root]');if(!button)return;event.stopPropagation();if(button.dataset.exportRoot!==undefined){exportBundle(button.dataset.exportRoot);return}const root=button.dataset.projectRoot||'';if(button.dataset.projectAction==='delete')deleteProjectDialog(root,button.dataset.projectName||'');else projectAction(root,button.dataset.projectAction)});
async function exportBundle(root){
  const r=await apiFetch('/v1/bundle/export',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({root})});
...
$('bundleImport')?.addEventListener('click',async()=>{
  ...
  try{const r=await apiFetch('/v1/bundle/import'+suffix,{method:'POST',headers:{'Content-Type':'application/zip'},body:await file.arrayBuffer()});const d=await r.json();$('bundleImportStatus').textContent=d.success?'Import successful: '+String(d.root||''):'Error: '+String(d.error||'failed')}
