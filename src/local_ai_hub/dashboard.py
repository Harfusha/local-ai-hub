...
function renderAgentLoopPresentation(model){
  ...
  const budget=traceRenderBudget(),presentation=traceSanitizeValue(model.presentation||{}),safeModel={...model,presentation},events=traceCodexEvents({...model,presentation},presentation),lifecycle=safeModel.lifecycle&&typeof safeModel.lifecycle==='object'?safeModel.lifecycle:{},input=safeModel.input??presentation.modelInput??'',objective=presentation.objective??safeModel.objective??(input&&typeof input==='object'?(input.prompt??input.objective??input.request??input):input),result=safeModel.output??presentation.modelOutput??'',error=safeModel.errors??presentation.error??'',state=traceInlineText(lifecycle.state??lifecycle.status??(events.length?'live':'empty'))||'empty';
  const toolRecords=[],toolById=new Map();
  events.forEach(event=>{const type=String(event?.event_type||'').toLowerCase(),payload=event?.payload&&typeof event.payload==='object'?tracePresentationValue(event.payload,8000,budget):{};if(type==='tool_call'){const record={call:payload,result:null};toolRecords.push(record);const id=payload?.call_id??payload?.callId;if(id)toolById.set(String(id),record);}else if(type==='tool_result'){const id=payload?.call_id??payload?.callId,record=id&&toolById.get(String(id));if(record)record.result=payload;else toolRecords.push({call:null,result:payload});}});
  const toolCards=toolRecords.length?toolRecords.slice(0,100).map((record,index)=>{const call=record.call||{},resultPayload=record.result||{},resultData=traceToolResultData(resultPayload,budget),failed=Boolean(resultData.error),name=call.name||call.tool||resultPayload.name||resultPayload.tool||'tool',status=resultPayload.status??resultPayload.state??call.status??(failed?'error':'pending'),summary=resultData.value?.result??resultData.value?.output??resultData.value?.error??'';return `<article class="trace-tool-card ${failed?'error':''}" data-tool-index="${index+1}"><h4>Tool ${index+1} · ${esc(traceInlineText(name))}</h4><div class="trace-tool-field"><strong>status</strong><div>${esc(traceInlineText(status))}</div></div>${traceRecorded(summary)?`<div class="trace-tool-field"><strong>${failed?'failure':'summary'}</strong>${traceHumanReadableMarkup(traceAgentLoopPrimaryValue(summary))}</div>`:'<div class="trace-primary-empty">No result captured</div>'}</article>`}).join(''):'<div class="trace-primary-empty" data-agent-state="empty">No tool calls captured</div>';
  const rawPayloads=traceAgentLoopPrimaryValue(toolRecords.map(record=>({call:record.call,result:record.result})));
  const rawMarkup=`<details class="trace-secondary-details trace-agent-raw" data-trace-collapsible><summary>Raw tool payloads</summary><div class="trace-optional-body">${renderAny(tracePresentationValue(rawPayloads,12000,budget))}</div></details>`;
  const metadataMarkup=`<details class="trace-secondary-details trace-agent-metadata" data-trace-collapsible><summary>Full event metadata</summary><div class="trace-optional-body">${renderAny(tracePresentationValue(traceAgentLoopPrimaryValue(events),12000,budget))}</div></details>`;
  const correlationMarkup=`<details class="trace-secondary-details trace-agent-correlations" data-trace-collapsible><summary>Correlations</summary><div class="trace-optional-body">${renderAny(tracePresentationValue(traceAgentLoopPrimaryValue(safeModel.correlations||{}),4000,budget))}</div></details>`;
  const timeline=traceCodexTimeline(safeModel,'Agent loop',null,true);
  const timelineMarkup=`<details class="trace-secondary-details trace-agent-timeline" data-trace-collapsible><summary>Execution timeline</summary><div class="trace-optional-body">${timeline}</div></details>`;
  const primary=`<section class="trace-primary trace-agent-loop-presentation"><div class="trace-primary-head"><h2>Agent loop</h2><span class="tiny">${esc(state)}</span></div><div class="trace-agent-state"><strong>Lifecycle</strong><span>${esc(state)}</span></div>${traceSummaryCard('Objective',objective,budget)}${traceSummaryCard('Model input',input,budget)}${traceSummaryCard('Final result',result,budget,{full:true})}${traceSummaryCard('Failure summary',traceRecorded(error)?error:'No failures recorded',budget,{tone:traceRecorded(error)?'error':''})}<section class="trace-tool-summary"><h3>Tool summary <span class="tiny">${toolRecords.length}</span></h3><div class="trace-tool-timeline">${toolCards}</div></section>${rawMarkup}${metadataMarkup}${correlationMarkup}${timelineMarkup}</section>`;
