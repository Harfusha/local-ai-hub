...
function traceCodexEvents(model,presentation){
  ...
  if(events.length){const hasOutput=events.some(event=>['output_delta','output_stream','assistant_output','model_output','output'].includes(String(event?.event_type||'').toLowerCase())),finalOutput=traceRecorded(presentation?.modelOutput)?presentation.modelOutput:traceRecorded(model?.session?.output)?model.session.output:null;if(!hasOutput&&traceRecorded(finalOutput))events.push({event_type:'assistant_output',payload:{content:traceRawBoundValue(finalOutput,8000)}});return events.slice(-100);}
