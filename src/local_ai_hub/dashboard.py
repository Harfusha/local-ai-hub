...
function renderCommandPresentation(model){
  const budget=traceRenderBudget(),presentation=model?.presentation&&typeof model.presentation==='object'?model.presentation:{},rawCommand=presentation.command&&typeof presentation.command==='object'&&!Array.isArray(presentation.command)?presentation.command:{},command=traceSanitizeValue(tracePresentationPayload(model,'command')||{}),pick=(keys,fallback='')=>tracePresentationPick(command,keys,fallback),cmdVal=pick(['command','cmd'],''),argsVal=pick(['args','arguments'],''),inputVal=pick(['input','stdin'],''),exitCodeVal=pick(['exit_code','exitCode','code'],''),successVal=pick(['success'],model?.success),retriesVal=pick(['retries','retry_count','attempts'],''),durationVal=pick(['duration','duration_ms','elapsed_ms'],''),errorVal=pick(['error'],model?.error??model?.errors??''),lifecycle=model?.lifecycle&&typeof model.lifecycle==='object'?model.lifecycle:{},lifecycleText=traceInlineText(lifecycle.state??lifecycle.status??'').toLowerCase(),failed=successVal===false||((typeof exitCodeVal==='number'||(typeof exitCodeVal==='string'&&exitCodeVal.trim()!==''))&&Number(exitCodeVal)!==0)||traceRecorded(errorVal)||/failed|failure|error/.test(lifecycleText),live=/queued|pending|running|active|processing|streaming|live|incomplete/.test(lifecycleText),commandHas=Object.entries(command).some(([key,value])=>traceMeaningfulValue(value,key)),state=failed?'failed':live?'live':!commandHas?'empty':successVal===true||Number(exitCodeVal)===0?'success':'incomplete',stateLabel=state==='failed'?'Failed':state==='live'?'Live / incomplete':state==='success'?'Success':state==='empty'?'Not captured':'Incomplete',stdout=pick(['stdout'],''),stderr=pick(['stderr'],''),stdoutPreview=traceRecorded(stdout)?traceCodeBlock('stdout',tracePresentationValue(stdout,1400),budget):'',stderrPreview=traceRecorded(stderr)?traceCodeBlock('stderr',tracePresentationValue(stderr,1400),budget):'',outputMarkup=stdoutPreview||stderrPreview?`${stdoutPreview}${stderrPreview}`:'<div class="trace-primary-empty">No output captured</div>',cmdSummary=traceMeaningfulValue(cmdVal)?`<div class="trace-command-line"><code>${esc(traceInlineText(cmdVal))}</code></div>`:'<div class="trace-primary-empty">No command captured</div>',argsSafe=tracePresentationValue(argsVal,3000),argsMarkup=Array.isArray(argsSafe)&&argsSafe.length?traceList('arguments',argsSafe,budget):traceMeaningfulValue(argsSafe)?traceSummaryCard('arguments',argsSafe,budget):'<div class="trace-primary-empty">No arguments captured</div>',stateMarkup=`<section class="trace-summary-card trace-command-state"><h3>exit state</h3><div class="trace-summary-value">${esc(stateLabel)}</div>${failed?'<div class="trace-primary-empty">Command failed</div>':''}${live?'<div class="trace-primary-empty">Command live / incomplete</div>':''}</section>`,failureMarkup=failed&&traceMeaningfulValue(errorVal)?traceSummaryCard('failure',errorVal,budget):'',top=`<section class="trace-summary-card trace-command-top"><h3>Command and arguments</h3>${cmdSummary}${argsMarkup}${stateMarkup}${failureMarkup}</section>`,left=[traceMeaningfulValue(inputVal)?traceSummaryCard('input',inputVal,budget):''],right=[outputMarkup,traceMeaningfulValue(exitCodeVal)?traceSummaryCard('exit code',exitCodeVal,budget):'',traceMeaningfulValue(durationVal)?traceSummaryCard('duration',durationVal,budget):''],details=[];
  if(state==='live')details.push('<div class="trace-primary-empty">Command is live or incomplete</div>');
  if(traceMeaningfulValue(rawCommand.cwd??command.cwd))details.push(traceSummaryCard('cwd',rawCommand.cwd??command.cwd,budget,{full:true}));
  if(traceMeaningfulValue(rawCommand.paths??command.paths))details.push(Array.isArray(rawCommand.paths??command.paths)?traceList('paths',rawCommand.paths??command.paths,budget):traceSummaryCard('paths',rawCommand.paths??command.paths,budget,{full:true}));
  if(traceMeaningfulValue(retriesVal))details.push(traceSummaryCard('retries',retriesVal,budget));
  if(traceMeaningfulValue(rawCommand.criterion??command.criterion))details.push(traceSummaryCard('criterion',rawCommand.criterion??command.criterion,budget,{full:true}));
  ...
  if(fullStdout)details.push(fullStdout);
  if(fullStderr)details.push(fullStderr);
...
function renderReviewPresentation(model){
  const budget=traceRenderBudget(),presentation=model?.presentation&&typeof model.presentation==='object'?model.presentation:{},rawReview=presentation.review&&typeof presentation.review==='object'&&!Array.isArray(presentation.review)?presentation.review:{},review=traceSanitizeValue(tracePresentationPayload(model,'review')||{}),requestRaw=rawReview.request??review.request??'',target=rawReview.target??rawReview.target_path??(requestRaw&&typeof requestRaw==='object'?requestRaw.target??requestRaw.path??requestRaw.repository:'')??'',scope=rawReview.scope??rawReview.files??(requestRaw&&typeof requestRaw==='object'?requestRaw.scope??requestRaw.files:'')??'',findingsRaw=rawReview.findings??review.findings??review.text??model?.output??'',findings=Array.isArray(findingsRaw)?findingsRaw:traceMeaningfulValue(findingsRaw)?[findingsRaw]:[],status=review.status??rawReview.status??model?.lifecycle?.state??'',errorVal=review.error??rawReview.error??model?.error??model?.errors??'',lifecycle=model?.lifecycle&&typeof model.lifecycle==='object'?model.lifecycle:{},lifecycleText=traceInlineText(lifecycle.state??lifecycle.status??'').toLowerCase(),failed=traceRecorded(errorVal)||review.success===false||rawReview.success===false||/failed|failure|error|blocked/.test(String(status).toLowerCase()),live=!failed&&/queued|pending|running|active|processing|streaming|live|incomplete/.test(lifecycleText||String(status).toLowerCase()),hasReview=traceRecorded(target)||traceRecorded(scope)||traceRecorded(requestRaw)||findings.length>0||traceRecorded(review.recommendation)||traceRecorded(status)||traceRecorded(errorVal),state=failed?'failed':live?'live':!hasReview?'empty':/complete|completed|done|success|succeeded|finished/.test(String(status).toLowerCase())?'complete':'incomplete',stateLabel=state==='failed'?'Review failed':state==='live'?'Review live / incomplete':state==='empty'?'Review not captured':state==='complete'?'Complete':'Incomplete',requestMarkup=traceMeaningfulValue(target)?traceSummaryCard('target',target,budget):traceMeaningfulValue(requestRaw)?traceSummaryCard('request',requestRaw,budget):'<div class="trace-primary-empty">Review target not captured</div>',scopeMarkup=traceMeaningfulValue(scope)?traceSummaryCard('scope',scope,budget):'',stateMarkup=`<section class="trace-summary-card trace-review-state"><h3>overall status</h3><div class="trace-summary-value">${esc(stateLabel)}</div>${failed?'<div class="trace-primary-empty">Review failed</div>':''}${live?'<div class="trace-primary-empty">Review live / incomplete</div>':''}</section>`,statusMarkup=traceMeaningfulValue(status)?traceSummaryCard('status',status,budget):'',severityMarkup=traceReviewSeverityMarkup(review.findings??findingsRaw,review.severity_counts??review.severityCounts,budget),groups=new Map();
  ...
  if(findings.length)details.push(`<section class="trace-summary-card"><h3>Full finding metadata</h3>${traceHumanReadableMarkup(tracePresentationValue(findings,12000))}</section>`);
