...
function tracePrimarySection(title,subtitle,markup){const content=String(markup??'');if(!content.trim())return '';const heading=esc(traceHumanText(title)),sub=traceRecorded(subtitle)?`<span class="tiny">${esc(traceHumanText(subtitle))}</span>`:'';return `<section class="trace-primary-section"><div class="trace-primary-head"><h2>${heading}</h2>${sub}</div>${content}</section>`;}
...
function tracePresentationShell(title,subtitle,primary,secondary,attention=''){const attentionMarkup=typeof attention==='string'&&attention.trim().startsWith('<section class="trace-attention-section"')?attention:traceAttentionSection(attention,traceRenderBudget(4000));return `${attentionMarkup}${tracePrimarySection(title,subtitle,primary)}${traceSecondaryDetails('Technical details',secondary)}`;}
...
function tracePresentationAttention(model,kind){
  ...
  if(kind==='model_chat'&&!traceRecorded(safe.output??safe.presentation?.modelOutput))items.push('Final response not captured');
