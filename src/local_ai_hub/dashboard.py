...
function traceRawBoundValue(value,limit=4096,depth=0){
  ...
  if(typeof value==='string'){const bounded=value.length>limit?value.slice(0,limit):value,room=budget?Math.min(limit,budget.remaining):limit,text=bounded.length>room?bounded.slice(0,room)+'… truncated':bounded;if(budget){budget.remaining-=text.length;if(text.length<value.length)budget.truncated=true;}return text;}
  if(depth>=4){if(budget)budget.truncated=true;return marker;}
...
function traceTrimPrimaryCard(markup,limit=240){const text=String(markup??'');if(text.length<=limit)return text;const match=text.match(/^(<section\b[^>]*><h3>[\s\S]*?<\/h3>)[\s\S]*?(<\/section>)$/);if(!match)return '<span class="trace-primary-empty">primary content truncated</span>';const inner=text.slice(match[1].length,text.length-match[2].length).replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim(),marker='… primary content truncated',room=Math.max(16,limit-match[1].length-match[2].length-marker.length-40);return `${match[1]}<div class="trace-summary-value">${esc(inner.slice(0,room))}${marker}</div>${match[2]}`;}
function traceAggregatePrimaryFields(fields,limit=22000){const values=(Array.isArray(fields)?fields:[]).filter(Boolean),text=values.join('');if(text.length<=limit)return values;const allocation=Math.max(240,Math.floor(limit/values.length));return values.map(field=>traceTrimPrimaryCard(field,allocation));}
...
function traceCodexTimeline(model,title='Model chat',renderBudget=null,full=false,materializedEvents=null){
  ...
  const cards=events.map((event,index)=>full?traceCodexEventMarkup(event,index,null,true):traceBudgetMarkup(traceCodexEventMarkup(event,index,budget),budget)).join(''),hasOutput=events.some(event=>['output_delta','output_stream','assistant_output','model_output','output'].includes(String(event?.event_type||'').toLowerCase())),outputState=traceCaptureState(presentation?.modelOutput,presentation?.finalResponseStatus??model.finalResponseStatus??model.session?.final_response_status??'');
...
function traceModelContextMarkup(input,budget){const source=input&&typeof input==='object'&&!Array.isArray(input)?input:{},details={};if(source.model!==undefined)details.model=source.model;if(source.stream!==undefined)details.stream=source.stream;if(source.options&&typeof source.options==='object')details.options=source.options;if(source.request&&typeof source.request==='object')details.request={path:source.request.path};if(source.headers||source.metadata||(source.request&&source.request.headers))details.security='Sensitive request metadata redacted';if(!Object.keys(details).length)return '';return `<details class="trace-optional-details trace-model-context" data-trace-key="model-context" data-trace-collapsible><summary>Model details · compact</summary><div class="trace-optional-body">${renderAny(tracePresentationValue(details,4000,budget))}</div></details>`;}
 function renderModelChatPresentation(model){
  ...
  const toolSummary=toolRecords.length?toolRecords.slice(0,100).map((record,index)=>{const call=record.call||{},result=record.result||{},resultData=traceToolResultData(result,budget),failed=Boolean(resultData.error),name=call.name||call.tool||result.name||result.tool||'tool',status=result.status??result.state??call.status??(failed?'error':'complete'),summary=resultData.value?.result??resultData.value?.output??resultData.value?.error??'';return `<div class="trace-tool-summary-row ${failed?'error':''}"><strong>${esc(traceInlineText(name))}</strong><span>${esc(traceInlineText(status))}</span>${traceRecorded(summary)?`<span>${esc(traceInlineText(summary))}</span>`:''}</div>`}).join(''):'<div class="trace-primary-empty">No tool calls captured</div>';
...
function renderAgentLoopPresentation(model){
  ...
  const primaryField=(label,value,emptyLabel)=>{const fieldBudget=traceRenderBudget(4000),valueState=traceCaptureState(value,''),safe=tracePresentationValue(value,4000,fieldBudget),hasValue=traceMeaningfulValue(safe),body=hasValue?traceHumanValue(safe,fieldBudget,4000):`<span class="trace-primary-empty">${esc(valueState==='empty'?`${label} empty`:emptyLabel)}</span>`;return `<section class="trace-summary-card trace-agent-field" data-field="${esc(String(label).toLowerCase().replace(/\s+/g,'-'))}"><h3>${esc(label)}</h3><div class="trace-summary-value">${body}</div></section>`;};
  const toolRecords=[],toolById=new Map();
  ...
  const toolCards=toolRecords.length?toolRecords.slice(0,100).map((record,index)=>{const call=record.call||{},resultPayload=record.result||{},resultData=traceToolResultData(resultPayload),failed=Boolean(resultData.error),name=call.name||call.tool||resultPayload.name||resultPayload.tool||'tool',status=resultPayload.status??resultPayload.state??call.status??(failed?'error':'pending'),summary=resultData.value?.result??resultData.value?.output??resultData.value?.error??'',summaryBudget=traceRenderBudget(4000),safeSummary=tracePresentationValue(traceAgentLoopPrimaryValue(summary),4000,summaryBudget);return `<article class="trace-tool-card ${failed?'error':''}" data-tool-index="${index+1}"><h4>Tool ${index+1} · ${esc(traceInlineText(name))}</h4><div class="trace-tool-field"><strong>status</strong><div>${esc(traceInlineText(status))}</div></div>${traceRecorded(safeSummary)?`<div class="trace-tool-field"><strong>${failed?'failure':'summary'}</strong>${traceHumanReadableMarkup(safeSummary)}</div>`:'<div class="trace-primary-empty">No result captured</div>'}</article>`}).join(''):'';
  const rawPayloads=traceAgentLoopPrimaryValue(toolRecords.map(record=>({call:record.call,result:record.result})));
  const rawMarkup=toolRecords.length?`<details class="trace-secondary-details trace-agent-raw" data-trace-collapsible><summary>Raw tool payloads</summary><div class="trace-optional-body">${renderAny(tracePresentationValue(rawPayloads,12000))}</div></details>`:'';
  const metadataMarkup=events.length?`<details class="trace-secondary-details trace-agent-metadata" data-trace-collapsible><summary>Full event metadata</summary><div class="trace-optional-body">${renderAny(tracePresentationValue(traceAgentLoopPrimaryValue(events),12000))}</div></details>`:'';
...
function tracePrimaryBoundedValue(value,limit=6000){const state=traceRenderBudget(limit),safe=tracePresentationValue(value,limit,state);return state.truncated&&typeof safe==='string'?`${safe}… truncated`:safe;}
...
function traceHumanValue(value,budget,limit=4000){
  ...
  if(Array.isArray(val)){
    ...
    return `<div class="trace-list-items">${items.map(it=>`<div>${traceHumanValue(it,budget,limit)}</div>`).join('')}</div>`;
  }
  if(typeof val==='object'){
    const entries=Object.entries(val).filter(([k,v])=>traceMeaningfulValue(v,k));
    if(!entries.length)return '';
    return entries.map(([k,v])=>`<div class="trace-obj-field"><strong class="trace-obj-key">${esc(k)}:</strong> <span class="trace-obj-val">${traceHumanValue(v,budget,limit)}</span></div>`).join('');
  }
  return esc(String(val));
}
...
function traceList(title,items,budget,renderItem){
  ...
  const rendered=renderItem?values.map((item,index)=>renderItem(item,index,budget)).join(''):values.map(item=>`<div class="trace-list-item">${traceHumanValue(item,budget)}</div>`).join('');
...
function renderReviewPresentation(model){
  ...
  const findingMarkupRaw=findings.length?Array.from(groups.entries()).map(([severity,items])=>`<section class="trace-finding-group trace-list-card" data-severity="${esc(severity)}"><h3>${esc(humanLabel(severity))} findings <span class="tiny">${items.length}</span></h3>${items.slice(0,50).map(item=>{const safe=bound(item,2600),summary=safe&&typeof safe==='object'?Object.fromEntries([['rule',safe.rule],['title',safe.title],['message',bound(safe.message??safe.description??safe.text,1600)]].filter(([,value])=>traceMeaningfulValue(value))):bound(safe,1600);return `<article class="trace-finding-item"><span class="trace-chip ${esc(severity)}">${esc(humanLabel(severity))}</span><div>${traceHumanReadableMarkup(summary)}</div></article>`}).join('')}</section>`).join(''):'<div class="trace-primary-empty">No findings recorded</div>',findingMarkup=traceBudgetMarkup(findingMarkupRaw,traceRenderBudget(12000)),failureMarkup=failed&&traceMeaningfulValue(errorVal)?traceSummaryCard('failure',bound(errorVal,1800),budget):'',recommendationMarkup=traceMeaningfulValue(recommendationSafe)?traceSummaryCard('recommendation',recommendationSafe,budget,{full:true}):'',findingMetadata=tracePresentationValue(findings.map(item=>{const safe=traceSanitizeValue(item);if(!safe||typeof safe!=='object')return bound(safe,1600);return Object.fromEntries([['severity',bound(safe.severity,300)],['rule',bound(safe.rule,500)],['title',bound(safe.title,500)],['message',bound(safe.message??safe.description??safe.text,4000)],['details',bound(safe.details,6000)]].filter(([,value])=>traceMeaningfulValue(value)));}),12000),primary=`<section class="trace-summary-card trace-review-top"><h3>Review target and scope</h3>${requestMarkup}${scopeMarkup}${stateMarkup}${severityMarkup}${findingMarkup}${recommendationMarkup}${failureMarkup}</section>`,details=[];
  ...
  if(Object.keys(rawSource).length)details.push(`<section class="trace-code-card"><h3>Raw review metadata</h3><pre class="trace-output">${esc(traceHumanText(traceRawBoundValue(traceSanitizeValue(rawSource),12000)))}</pre></section>`);
...
function renderRepoIntelligencePresentation(model){
  ...
  if(Object.keys(rawSource).length){const raw=traceRawBoundValue(traceSanitizeValue(cleanPayload(rawSource)),12000);details.push(`<section class="trace-code-card"><h3>Full repository payload</h3><pre class="trace-output">${esc(traceHumanText(raw))}</pre></section>`);}
...
function renderRagSearchPresentation(model){
  const budget=traceRenderBudget(),retrieval=traceSanitizeValue(tracePresentationPayload(model,'retrieval')||{}),query=tracePresentationPick(retrieval,['query','search_query','term'],''),hasResultsField=Object.prototype.hasOwnProperty.call(retrieval,'results'),malformedResults=hasResultsField&&!Array.isArray(retrieval.results),rawResults=Array.isArray(retrieval.results)?retrieval.results:(Array.isArray(retrieval.hits)?retrieval.hits:[]),sources=Array.isArray(retrieval.sources)?retrieval.sources:[],results=rawResults.length?rawResults:sources,ranked=results.map((item,index)=>({item,index,score:Number(item?.score??item?.relevance??item?.similarity)})).sort((a,b)=>{const af=Number.isFinite(a.score),bf=Number.isFinite(b.score);if(af&&bf)return b.score-a.score||a.index-b.index;if(af!==bf)return af?-1:1;return a.index-b.index}),limit=50,shown=ranked.slice(0,limit),resultLabel=`${results.length}${results.length===1?' result':' results'}`,answer=tracePresentationPick(retrieval,['answer','response','summary'],''),truncatedValue=retrieval.truncated??retrieval.truncation??retrieval.results_truncated??retrieval.resultsTruncated,hasTruncation=Boolean(truncatedValue)||results.length>limit;
  const aggregateLimit=24000;let presentationTruncated=false;const boundPresentation=(value,limit)=>{const safe=traceSanitizeValue(value),bounded=traceRawBoundValue(safe,limit);let rawLength=0;try{const encoded=typeof safe==='string'?safe:JSON.stringify(safe);rawLength=String(encoded??'').length;}catch(error){rawLength=limit+1;}if(rawLength>limit)presentationTruncated=true;return bounded;},querySafe=boundPresentation(query,2400),answerSafe=boundPresentation(answer,5000);
  const cleanPayload=(value,key='')=>{const normalized=String(key||'').replace(/([a-z])([A-Z])/g,'$1_$2').toLowerCase();if(traceSensitiveField(key)||/^(evidence_id|file_sha256|content_hash|sha256)$/.test(normalized))return undefined;if(Array.isArray(value))return value.map(item=>cleanPayload(item)).filter(item=>item!==undefined);if(value&&typeof value==='object'){const cleaned={};Object.entries(value).forEach(([childKey,childValue])=>{const child=cleanPayload(childValue,childKey);if(child!==undefined)cleaned[childKey]=child});return cleaned;}return value;};
  const metadata=[['project',traceProjectLabel(model)],['root',retrieval.root],['mode',retrieval.mode],['provider',retrieval.provider],['limit',retrieval.top_k??retrieval.limit]].filter(([,value])=>traceMeaningfulValue(value)).map(([label,value])=>traceSummaryCard(label,boundPresentation(value,1800),budget,{full:true})).join('');
  const resultRow=(entry,index)=>{const item=entry.item,source=item&&typeof item==='object'&&!Array.isArray(item)?item:{},title=traceSearchText(source.title??source.name??source.path??source.file??source.filename??item,320)||`Result ${index+1}`,provider=traceSearchText(source.provider??source.source??source.repository??'',240),snippet=traceSearchText(source.snippet??source.text??source.content??source.summary??source.description??item,900),path=traceSearchText(source.path??source.file??source.filename??'',1200),start=source.start_line??source.startLine,end=source.end_line??source.endLine,location=path&&start!==undefined?`${path} · line ${traceInlineText(start)}${end!==undefined&&traceInlineText(end)!==traceInlineText(start)?`–${traceInlineText(end)}`:''}`:path,score=Number.isFinite(entry.score)?String(entry.score):'',payload=traceRawBoundValue(traceSanitizeValue(cleanPayload(item)),8000),extra=[score?traceSummaryCard('score',score,budget):'',provider?traceSummaryCard('provider',provider,budget):'',location?traceSummaryCard('path',location,budget):'',`<section class="trace-code-card"><h3>Full result payload</h3><pre class="trace-output">${esc(traceHumanText(payload))}</pre></section>`].filter(Boolean).join('');return `<article class="trace-search-result-row"><div class="trace-search-rank">${esc(index+1)}</div><div class="trace-search-result-main"><strong>${esc(title)}</strong>${provider?`<span class="tiny">${esc(provider)}</span>`:''}${snippet?`<div class="trace-search-snippet">${esc(snippet)}</div>`:'<div class="trace-primary-empty">No snippet captured</div>'}<details class="trace-secondary-details trace-rag-result-details" data-trace-collapsible><summary>More result details</summary><div class="trace-optional-body">${extra}</div></details></div></article>`;};
  const rowAggregateLimit=9000;let rows='',renderedCount=0;for(const entry of shown){const candidate=resultRow(entry,renderedCount);if(rows&&rows.length+candidate.length>rowAggregateLimit)break;rows+=candidate;renderedCount+=1;}const aggregateTruncated=renderedCount<shown.length||results.length>limit||presentationTruncated,aggregateMarkup=aggregateTruncated?`<div class="trace-summary-card trace-search-truncation"><h3>Aggregate output truncated</h3><div>${presentationTruncated?'Large fields were bounded; ':''}Showing ${esc(String(renderedCount))} of ${esc(String(results.length))} ranked results within the ${esc(String(rowAggregateLimit))}-character result budget.</div></div>`:'';
  const resultMarkup=rows||'<div class="trace-primary-empty">No results found</div>',answerMarkup=traceRecorded(answerSafe)?traceSummaryCard('answer',answerSafe,budget,{full:true}):'<div class="trace-primary-empty">No answer captured</div>',truncationMarkup=(hasTruncation||aggregateTruncated)?`<div class="trace-summary-card trace-search-truncation"><h3>Truncation · truncated results</h3><div>${esc(traceSearchText(truncatedValue||`Showing ${renderedCount} of ${results.length} results`,420))}</div></div>${aggregateMarkup}`:aggregateMarkup,malformedMarkup=malformedResults?'<div class="trace-summary-card trace-search-malformed"><h3>Malformed results</h3><div class="trace-primary-empty">Results payload must be an array</div></div>':'';
  const primary=`<section class="trace-primary trace-search-presentation"><div class="trace-primary-head"><h2>Search</h2><span class="tiny">${esc(resultLabel)}</span></div>${traceSummaryCard('Query',querySafe,budget,{key:'query'})||'<div class="trace-summary-card"><h3>Query</h3><div class="trace-primary-empty">No query captured</div></div>'}${answerMarkup}<div class="trace-summary-card trace-search-count"><h3>result count</h3><div class="trace-summary-value">${esc(String(results.length))}</div></div>${malformedMarkup}${truncationMarkup}<div class="trace-list-card trace-search-results"><h3>ranked results</h3>${resultMarkup}</div></section>`;
  const secondary=metadata?traceSecondaryDetails('Search metadata',metadata):'';
  const aggregateMarker=`<div class="trace-summary-card trace-search-aggregate-limit"><h3>Aggregate output truncated</h3><div>Technical details omitted to keep this trace within the ${aggregateLimit}-character output budget.</div></div>`;
  const fullMarkup=`${primary}${secondary}`;
  if(fullMarkup.length<=aggregateLimit)return traceFinalizeMarkup(fullMarkup,budget);
  if(primary.length+aggregateMarker.length<=aggregateLimit)return traceFinalizeMarkup(`${primary}${aggregateMarker}`,budget);
  return traceFinalizeMarkup(`${primary.slice(0,Math.max(0,aggregateLimit-aggregateMarker.length))}${aggregateMarker}`,budget);
}
...
function traceWrapPresentationRenderer(renderer,title,subtitle,kind,model){const budget=traceRenderBudget(),primary=renderer(model),secondary=tracePresentationField('Capture state',model?.captureStates||{},'',budget,{mode:'technical'});return tracePresentationShell(title,subtitle,primary,secondary,tracePresentationAttention(model,kind));}
