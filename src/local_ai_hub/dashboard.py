...
function traceReviewSeverityMarkup(findings,budget){const counts={critical:0,high:0,medium:0,low:0},items=Array.isArray(findings)?findings:(findings&&typeof findings==='object'?Object.values(findings):[]);items.forEach(item=>{const severity=String(item?.severity||item?.priority||'low').toLowerCase();if(counts[severity]!==undefined)counts[severity]+=1;});return `<div class="trace-severity-counts" aria-label="severity counts">${Object.entries(counts).map(([level,count])=>`<span class="trace-event-chip ${count?'trace-severity-active':''}">${esc(level)}: ${count}</span>`).join('')}</div>`;}
function tracePresentationRankedField(label,value,empty,budget){const raw=Array.isArray(value)?value:[];if(!raw.length)return `<div class="trace-presentation-field"><strong>${esc(label)}</strong><span class="trace-primary-empty">${esc(empty)}</span></div>`;const ranked=raw.map((item,index)=>({item,index,score:Number(item?.score??item?.relevance??item?.similarity),rank:Number(item?.rank??index+1)})).sort((a,b)=>Number.isFinite(b.score)&&Number.isFinite(a.score)?b.score-a.score:a.rank-b.rank);const markup=ranked.slice(0,20).map((entry,index)=>{const safe=tracePresentationValue(entry.item,1800,budget),score=Number.isFinite(entry.score)?entry.score:'—',rank=Number.isFinite(entry.rank)?entry.rank:index+1,provider=entry.item?.provider||entry.item?.source||'—';return `<details class="trace-rag-source-details"><summary>rank ${esc(rank)} · score ${esc(score)} · provider ${esc(provider)}</summary>${renderAny(safe)}</details>`}).join('');return `<div class="trace-presentation-field trace-rag-ranked"><strong>${esc(label)}</strong><div class="trace-rag-ranked">${markup}</div></div>`;}
...
function renderReviewPresentation(model){
  ...
  return tracePresentationColumns('Review',left,right,budget,traceReviewSeverityMarkup(review.findings,budget));
}
