...
function traceCodexEventMarkup(event,index,budget,hideInternal=false){
  ...
  let body='',extra='';
  if(p?.malformed_payload)return `<div class="trace-timeline-event trace-malformed"><div class="trace-timeline-marker">Malformed event payload${hideInternal?'':` · step ${esc(step)}`}</div><div class="trace-primary-empty">Malformed event payload</div>${renderAny(tracePresentationValue(p.value,2048,budget))}</div>`;
  if(isThinking){const content=p.content??p.text??p.reasoning??p.thinking??p.output??p,thinkingKey=`${step}:${event?.seq??index}`,position=hideInternal?'':` · step ${esc(step)}`;return `<div class="trace-timeline-event trace-thinking"><details class="trace-thinking" data-thinking-key="${esc(thinkingKey)}"><summary>${esc(label)}${position} · collapsed</summary>${promptBody(tracePresentationValue(content,8000,budget))}</details></div>`;}
  if(isRequest){const input=p.messages??p.prompt??p.content??p.input??p.request??p;body=traceModelInputMessages(input,{step}).map((message,messageIndex)=>traceChatMessageMarkup(message,messageIndex,budget,hideInternal)).join('');}
  ...
  else if(isOutput){body=`<article class="trace-chat-message assistant"><div class="trace-chat-role">assistant${hideInternal?'':` · step ${esc(step)}`}</div>${traceReadableMarkup(p.text??p.content??p.output??p.result??p,budget,8000)}</article>`;}
