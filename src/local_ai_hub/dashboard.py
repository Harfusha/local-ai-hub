...
function traceRawBoundValue(value,limit=4096,depth=0){
  ...
  if(typeof value==='object'){const entries=Object.entries(value).slice(0,50),childLimit=Math.max(128,Math.floor(limit/Math.max(entries.length,1))),result={};for(const [key,item] of entries){if(budget&&budget.remaining<=0){budget.truncated=true;break}if(budget)budget.remaining-=Math.min(budget.remaining,key.length+8);result[key]=traceRawBoundValue(item,childLimit,depth+1,budget);}if(Object.keys(value).length>entries.length||budget?.truncated)result['…']=marker;return result;}
...
function tracePresentationBound(value,budget,depth=0){
  ...
  if(typeof value==='object'){
    ...
    for(const [key,item] of entries){if(budget.remaining<=0){budget.truncated=true;break}budget.remaining-=Math.min(budget.remaining,key.length+8);result[key]=tracePresentationBound(item,budget,depth+1)}
...
function traceModelContextMarkup(input,budget){const source=input&&typeof input==='object'&&!Array.isArray(input)?input:{},details={};if(source.model!==undefined)details.model=source.model;if(source.stream!==undefined)details.stream=source.stream;if(source.options&&typeof source.options==='object')details.options=source.options;if(source.request&&typeof source.request==='object')details.request={path:source.request.path};if(source.headers||source.metadata||(source.request&&source.request.headers))details.security='Sensitive request metadata redacted';if(!Object.keys(details).length)return '';return `<details class="trace-optional-details trace-model-context" data-trace-key="model-context" data-trace-collapsible><summary>Model details · compact</summary><div class="trace-optional-body">${renderAny(tracePresentationValue(details,4000,budget))}</div></details>`;}
 function renderModelChatPresentation(model){
  ...
  timelineEvents.forEach(event=>{const type=String(event?.event_type||'').toLowerCase(),payload=event?.payload&&typeof event.payload==='object'?tracePresentationValue(event.payload,6000,budget):{};if(type==='tool_call'){const record={call:payload,result:null};toolRecords.push(record);const id=payload?.call_id??payload?.callId;if(id)toolById.set(String(id),record);}else if(type==='tool_result'){const id=payload?.call_id??payload?.callId,record=id&&toolById.get(String(id));if(record)record.result=payload;else toolRecords.push({call:null,result:payload});}});
  const toolSummary=toolRecords.length?traceMarkupItems(toolRecords.slice(0,100).map((record,index)=>{const call=record.call||{},result=record.result||{},resultData=traceToolResultData(result,budget),failed=Boolean(resultData.error),name=call.name||call.tool||result.name||result.tool||'tool',status=result.status??result.state??(failed?'error':record.result?'complete':call.status??'pending'),summary=traceAgentLoopPrimaryValue(resultData.value?.result??resultData.value?.output??resultData.value?.error??'');return `<div class="trace-tool-summary-row ${failed?'error':''}"><strong>${esc(traceInlineText(name))}</strong><span>${esc(traceInlineText(status))}</span>${traceRecorded(summary)?`<span>${esc(traceInlineText(summary))}</span>`:''}</div>`}),6000):'<div class="trace-primary-empty">No tool calls captured</div>';
...
function renderAgentLoopPresentation(model){
  ...
  const correlationUnsafe=key=>/(^|_)(id|ids|request_id|correlation_id|call_id|session_id|job_id|lease_id|seq|sequence|header|headers|authorization|token|secret|password|private_key)(_|$)/.test(String(key).replace(/([a-z])([A-Z])/g,'$1_$2').toLowerCase());
...
function renderRepoIntelligencePresentation(model){
  ...
  const details=[],secondaryFields=[['context',repo.context],['graph',repo.graph??repo.graph_data??repo.graphData],['evidence',repo.evidence],['callers',repo.callers],['related symbols',repo.related_symbols??repo.relatedSymbols],['diagnostics',repo.diagnostics]];
...
function traceUnmappedPresentation(model,kind){
  ...
  const known=new Set(Object.keys(tracePresentationPayload(model,key))),budget=traceRenderBudget(),cards=[];
  for(const [label,source] of [['input',model.input],['output',model.output],['response',model.response],['error',kind==='rag_search'?model.errors:undefined]]){
    ...
    if(scalar&&((label==='output'||label==='response')&&['review','repo_intelligence','async_job'].includes(kind)))continue;
