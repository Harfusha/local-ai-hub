...
function renderAsyncJobPresentation(model){
  ...
  const stateText=traceInlineText(payload.status??lifecycle.state??lifecycle.status??'').toLowerCase(),terminal=lifecycle.terminal===true||/complete|completed|success|succeeded|failed|error|cancelled|canceled|interrupted/.test(stateText),liveState=terminal?'finished':(/queued|pending|waiting|running|active|processing/.test(stateText)||lifecycle.terminal===false?'live / incomplete':'incomplete');
  ...
  const workerPayload=tracePresentationPick(payload,['worker_input','worker_output'],''),correlations=safeModel.correlations&&typeof safeModel.correlations==='object'?safeModel.correlations:'',secondary=[workerPayload?traceSecondaryDetails('Worker payload',tracePresentationField('worker input',workerPayload,'',traceRenderBudget(),{mode:'technical'})):'',Object.keys(correlations||{}).length?traceSecondaryDetails('Correlations',tracePresentationField('correlations',correlations,'',traceRenderBudget(),{mode:'technical'})):'',Array.isArray(safeModel.events)&&safeModel.events.length?traceSecondaryDetails('Event timeline',traceCodexTimeline(safeModel,'Async job',traceRenderBudget(),true,safeModel.events)):''].join('');
  const left=[traceMeaningfulValue(jobType)?traceSummaryCard('job type',jobType,budget):'',traceMeaningfulValue(liveState)?traceSummaryCard('capture state',liveState,budget):'',traceMeaningfulValue(status)?traceSummaryCard('status',status,budget):'',traceMeaningfulValue(lifecycle.state)&&String(lifecycle.state)!==String(status)?traceSummaryCard('lifecycle',lifecycle.state,budget):'',traceMeaningfulValue(progress)?traceSummaryCard('progress',progress,budget):'',traceMeaningfulValue(queueWait)?traceSummaryCard('queue wait',queueWait,budget):'',traceMeaningfulValue(retries)?traceSummaryCard('retries',retries,budget):''];
  const right=[traceMeaningfulValue(result)?traceCodeBlock('result',result,budget):'',traceMeaningfulValue(error)?traceSummaryCard('error',error,budget):''];
  return tracePresentationColumns('Async job',left,right,budget)+secondary;
}
function renderRequestResponsePresentation(model){
  ...
  const left=[
     traceMeaningfulValue(reqVal)?traceSummaryCard('request',reqVal,budget):'',
    traceMeaningfulValue(requestBody)?traceSummaryCard('request body',requestBody,budget):'',
    traceRecorded(duration)?traceSummaryCard('duration',duration,budget):''
  ];
  const right=[
     traceMeaningfulValue(responseBody)?(typeof responseBody==='string'?traceCodeBlock('response',responseBody,budget,{key:'response'}):traceSummaryCard('response',responseBody,budget,{full:true})):'',
    traceMeaningfulValue(outputBody)?traceCodeBlock('output',outputBody,budget):'',
  ...
  return tracePresentationColumns('Request / response',left,right,budget)+technical;
}
