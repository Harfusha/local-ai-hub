...
function renderRagSearchPresentation(model){
  ...
  const aggregateLimit=24000;let rows='',renderedCount=0;for(const entry of shown){const candidate=resultRow(entry,renderedCount);if(rows&&rows.length+candidate.length>aggregateLimit)break;rows+=candidate;renderedCount+=1;}const aggregateTruncated=renderedCount<shown.length||results.length>limit,aggregateMarkup=aggregateTruncated?`<div class="trace-summary-card trace-search-truncation"><h3>Aggregate output truncated</h3><div>Showing ${esc(String(renderedCount))} of ${esc(String(results.length))} ranked results within the ${esc(String(aggregateLimit))}-character budget.</div></div>`:'';
  const resultMarkup=rows||'<div class="trace-primary-empty">No results found</div>',answerMarkup=traceRecorded(answer)?traceSummaryCard('answer',answer,budget,{full:true}):'<div class="trace-primary-empty">No answer captured</div>',truncationMarkup=(hasTruncation||aggregateTruncated)?`<div class="trace-summary-card trace-search-truncation"><h3>Truncation · truncated results</h3><div>${esc(traceSearchText(truncatedValue||`Showing ${renderedCount} of ${results.length} results`,420))}</div></div>${aggregateMarkup}`:aggregateMarkup,malformedMarkup=malformedResults?'<div class="trace-summary-card trace-search-malformed"><h3>Malformed results</h3><div class="trace-primary-empty">Results payload must be an array</div></div>':'';
  const primary=`<section class="trace-primary trace-search-presentation"><div class="trace-primary-head"><h2>Search</h2><span class="tiny">${esc(resultLabel)}</span></div>${traceSummaryCard('Query',query,budget,{key:'query'})||'<div class="trace-summary-card"><h3>Query</h3><div class="trace-primary-empty">No query captured</div></div>'}${answerMarkup}<div class="trace-summary-card trace-search-count"><h3>result count</h3><div class="trace-summary-value">${esc(String(results.length))}</div></div>${malformedMarkup}${truncationMarkup}<div class="trace-list-card trace-search-results"><h3>ranked results</h3>${resultMarkup}</div></section>`;
  const metadata=[['project',traceProjectLabel(model)],['root',retrieval.root],['mode',retrieval.mode],['provider',retrieval.provider],['limit',retrieval.top_k??retrieval.limit]].filter(([,value])=>traceMeaningfulValue(value)).map(([label,value])=>traceSummaryCard(label,value,budget,{full:true})).join('');
