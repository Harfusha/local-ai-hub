...
TOKEN_ECONOMIZER_SKILL_MD = TOKEN_ECONOMIZER_SKILL_MD.replace(
    "Universal terminal wrapper: runs commands, strips ANSI codes, truncates large output dumps to first/last N lines.",
...
def generate_token_economy_policy(cfg: dict[str, Any] | None = None) -> str:
    """Return the standard policy with the installed read-only CLI contract."""
    policy = _base_token_economy_policy(cfg)
    policy = policy.replace("`repomix --compress`", "`repomix --stdout --compress`")
    return policy.replace(
        "- Bounded command outputs: Filter test and build output (`trim-run <cmd>`, `pytest -q --tb=short`, `dotnet test --verbosity quiet`) or route through `local_ai_command`.",
        "- Bounded command outputs: Use `trim-run` only with commands in its safe read/validation/build allowlist, or pipe output into stdin mode. Use minimal test flags (`pytest -q --tb=short`, `dotnet test --verbosity quiet`) or route through `local_ai_command`.",
    ).replace(
        "- Context compression & token measurement: Use `repomix --compress`",
        "- Context compression & token measurement: Use `repomix --stdout --compress`",
    ).replace(
        "- Bounded command outputs:",
        "- Installed CLI defaults: Use `tokcount`, `repo-map`, `grep-ast`, `files-to-prompt`, `ast-grep`/`sg`, `repomix --stdout --compress`, `rg`, `fd`, and `jq`; the installer adds the token-economy scripts directory to PATH.\n- Bounded command outputs:",
    )
