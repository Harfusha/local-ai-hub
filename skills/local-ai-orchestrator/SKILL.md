...
- Use `local_ai_task` for bounded local-model work when local inference is the right fit. Use `qwen2.5-coder:0.5b` only for preprocessing, `qwen2.5-coder:1.5b` for quick tasks, `qwen2.5-coder:3b` for complex tasks, and `qwen2.5-coder:7b` for the hardest reasoning.
...
- **Four model tiers:** use `qwen2.5-coder:0.5b` only for preprocessing, `qwen2.5-coder:1.5b` for quick work, `qwen2.5-coder:3b` for complex tasks, and `qwen2.5-coder:7b` for the hardest reasoning after deterministic/indexed evidence.
...
9. `local_ai_task(action="delegate"|"reason"|"review"|"second_opinion"|"compress")` — use 1.5B for quick work, 3B for complex tasks, and 7B for hardest reasoning; 0.5B is preprocessing-only.
