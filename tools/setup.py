...
def ensure_user_path_entry(path: Path) -> bool:
    """Add an installed CLI directory to the current user's PATH idempotently."""
    ...
    if os.name == "nt":
        ...
        try:
            ...
            key = winreg.CreateKeyEx(winreg.HKEY_CURRENT_USER, "Environment", 0, winreg.KEY_READ | winreg.KEY_SET_VALUE)
        with key:
            try:
                user_path, value_type = winreg.QueryValueEx(key, "Path")
            except FileNotFoundError:
                user_path, value_type = "", winreg.REG_EXPAND_SZ
            entries = [entry for entry in user_path.split(os.pathsep) if entry]
            if not any(normalise(entry) == normalise(resolved) for entry in entries):
                entries.append(resolved)
                winreg.SetValueEx(key, "Path", 0, value_type, os.pathsep.join(entries))
                changed = True
    else:
        begin = "# BEGIN LOCAL AI HUB TOKEN ECONOMY PATH"
        end = "# END LOCAL AI HUB TOKEN ECONOMY PATH"
        block = f'{begin}\nexport PATH={shlex.quote(resolved)}:"$PATH"\n{end}'
        pattern = re.compile(rf"(?ms)^{re.escape(begin)}\n.*?^{re.escape(end)}$")
        for profile in (Path.home() / ".profile", Path.home() / ".zprofile"):
            content = profile.read_text(encoding="utf-8") if profile.is_file() else ""
            updated = pattern.sub(block, content).rstrip()
            if updated == content.rstrip():
                continue
            profile.parent.mkdir(parents=True, exist_ok=True)
            profile.write_text(f"{updated}\n\n{block}\n" if updated else f"{block}\n", encoding="utf-8")
            changed = True

    if not any(normalise(entry) == normalise(resolved) for entry in current.split(os.pathsep) if entry):
        os.environ["PATH"] = os.pathsep.join(part for part in (resolved, current) if part)
        changed = True
    return changed
...
def install_token_economy_suite(install_dir: Path, hub_python: Path, *, allow_external_tools: bool = True) -> None:
    ...
    if scripts_dir.is_dir() and ensure_user_path_entry(scripts_dir):
        log(f"Added token-economy commands to the current user's PATH: {scripts_dir}; restart the agent host to refresh its environment.")
    ...
    if shutil.which("npm"):
        try:
            npm_prefix = run(["npm", "config", "get", "prefix"], check=False, timeout=15, capture=True).stdout.strip()
            npm_bin_dir = Path(npm_prefix) if os.name == "nt" else Path(npm_prefix) / "bin"
            if npm_prefix and npm_bin_dir.is_dir() and ensure_user_path_entry(npm_bin_dir):
                log(f"Added npm's global CLI directory to the current user's PATH: {npm_bin_dir}; restart the agent host to refresh its environment.")
        except Exception as exc:
            log(f"Optional npm CLI PATH setup skipped: {exc}")
