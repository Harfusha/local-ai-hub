...
def main()->int:
    ...
    while True:
        ...
        if a.json_lines:
            ...
            clear(); q=status.get('scheduler') or {}; o=status.get('observability') or {}; p=status.get('preprocessing') or {}; h=status.get('headless') or {}
            ...
            print(f"Model: {q.get('active_model') or 'idle'}  Queue: {q.get('queued',0) or 0}  Inflight: {q.get('inflight',0) or 0}  Active requests: {o.get('active_request_count',0) or 0}  Background: {q.get('background_queued',0) or 0}")
            print(f"Cache hit: {100*float(o.get('cache_hit_rate') or 0):.1f}%  p95: {float(o.get('p95_duration_ms') or 0):.0f} ms  Net cloud delta: {int(o.get('net_cloud_token_delta_est') or 0):,}  Fallbacks: {o.get('fallback_count',0) or 0}")
            print(f"Token accounting: gross={int(o.get('gross_cloud_tokens_avoided_est') or 0):,}  protocol={int(o.get('agent_protocol_tokens_est') or 0):,} (call={int(o.get('agent_tool_request_tokens_est') or 0):,}, read={int(o.get('agent_tool_response_tokens_est') or 0):,})  schema≈{int(o.get('tool_schema_tokens_exposure_est') or 0):,}  local-compute={int(o.get('local_compute_tokens_avoided_est') or 0):,}")
            print(f"Preprocess: {'paused' if p.get('paused') else 'active'} · steps={p.get('steps',0) or 0} errors={p.get('errors',0) or 0} yields={p.get('yields',0) or 0}")
            for pr in (p.get('projects') or []):
                tot = max(1, int(pr.get('files') or 0))
                rag_f = int(pr.get('rag_files') or 0)
                card_f = int(pr.get('file_cards') or 0)
                overall = float(pr.get('overall_progress_pct') or 0)
            ...
            print('\nRecent activity:')
            for e in list(live.get('events') or [])[-18:]:
                ts=time.strftime('%H:%M:%S',time.localtime(float(e.get('created_at') or 0)))
